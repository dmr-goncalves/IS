 ...
 * @version $Date: 2004-09-22 15:08:42 +0200 (mer, 22 set 2004) $ $Revision:
 * 5374 $
 */
/**
 * This is the basic class that implements a behaviour of JADE that allows to
 * embed a Jess engine inside the agent code.
 * <p>
 ...
 * <p>
 * The programmer can override this class. In particular, its methods
 * <code>ACL2JessString</code> and <code>JessFact2ACL</code> to convert between
 * ACLMessage JADE structure and Jess facts. Also the method
 * <code>ACLJessTemplate</code> might need to be overide in order to change the
 * deftemplate of the ACLMessage in Jess.
 * <p>
 * When this behaviour is added to the list of agent behaviours, it creates a
 * Jess engine and initializes the engine by:
 * <ul>
 ...
 * <li> calling the <code>ACL2JessString</code> method that returns the fact to
 * be asserted in Jess,
 ...
 * <li> match the fact <code>(MyAgent (name nameofthisagent))</code> to know the
 * name of your agent;
 * <li> use the userfunction <code>send</code> to send ACLMessages. The
 * parameter of <code>send</code> must be a fact-id of type ACLMessage or an
 * ACLMessage itself; There are two styles of usage:
 * <p>
 * <code>  ?m <- (ACLMessage (communicative-act cfp) (sender ?s))
 * <br> (send ?m) </code>
 * <p>
 * or, in alternative,
 * <p>
 * <code>(send (assert (ACLMessage (communicative-act cfp)
 * (sender ?s))))</code>
 ...
 * <p>
 * WARNING: FIPA2000 standard has specified an AgentIdentifier to have a
 * template composed of several slots, where the only mandatory one is the agent
 * name, i.e. the globally unique identifier of the agent (GUID). In order to
 * reduce the porting of the JESS user code, this basic behaviour automatically
...
public class BasicJessBehaviour extends CyclicBehaviour {
    ...
    /**
     * Creates a <code>BasicJessBehaviour</code> instance that limits the
     * reasoning time of Jess before looking again for arrival of messages.
     *
     * @param agent the agent that adds the behaviour
     * @param jessFile the name of the Jess file to be executed
     * @param maxJessPasses the maximum number of passes that every run of Jess
     * can execute before giving again the control to this behaviour; put
     * <code>0</code> if you do not ever want to stop Jess.
     */
     ...
     * list of agent names.
     *
     * @param context represents the Rete engine context needed to resolve the
     * value of JESS variables
