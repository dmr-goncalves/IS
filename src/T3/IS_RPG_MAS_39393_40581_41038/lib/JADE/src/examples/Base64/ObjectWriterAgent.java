...
/**
 * This agent makes the following task: 1. searches in the DF for an
 * ObjectReaderAgent; 2. sends an ACLMessage with a content encoded in Base64 to
 * the ObjectReaderAgent. 3. sends the same message by using the BitEfficient
 * ACLCodec first, and the XML ACLCodec then 4. sends an ACLMessage with a
 * content encoded as a String 5. sends the same message by using again
 * BitfficientACLCodec and XMLACLCodec
 *
 * Notice that JADE actually encodes ACLMessages only and only if the receiver
 * of the message belongs to a remote platform. Notice that the
 * ObjectReaderAgent does not need any special provision to read bit-efficient
 * or XML-coded messages because all the burden is put into the platform that
 * most provide the proper decoding capabilities. See also the bitefficient
 * tutorial and the XMLACL tutorial that describe how to launch the platform
 * with this codec capabilities.
 *
 * Because this agent implements a single sequential task, it does not use any
 * Behaviour.
 *
 * @author Fabio Bellifemine - CSELT S.p.A
 * @version $Date: 2001-12-06 11:51:17 +0100 (gio, 06 dic 2001) $ $Revision:
 * 2911 $
 */
