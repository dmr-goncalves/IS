...
/**
 * This example shows how an agent can declare an ALIAS, i.e. an additional
 * name. Messages sent to that ALIAS will be delivered to the declaring agent as
 * if they were explicitly directed to it. In particular in this example the
 * Alias is constructed by adding the "-AAA" suffix to the agent name.<br>
 * In order to try the example, start an AliasAgent agent and then using the
 * DummyAgent tool send it a message specifying its alias as receiver.
 */
