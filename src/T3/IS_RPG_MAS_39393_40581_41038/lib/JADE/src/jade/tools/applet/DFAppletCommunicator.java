...
/**
 * This class is used by DFApplet in order to communicate with the DF agent via
 * a socket connection. The socket server is implemented by the agent
 * jade.tools.SocketProxyAgent.
 *
 * @see jade.tools.SocketProxyAgent.SocketProxyAgent
 *
 * @author Fabio Bellifemine - CSELT - 25/8/1999
 * @version $Date: 2004-07-19 17:54:06 +0200 (lun, 19 lug 2004) $ $Revision:
 * 5217 $
 */
public class DFAppletCommunicator implements DFGUIAdapter {
    ...
    /**
     * Create a socket to communicate with a server on port 6789 of the host
     * that the applet's code is on. Create streams to use with the socket.
     * Finally, gets the value of the parameter <code>JADEAddress</code> from
     * the HTML file.
     */
    ...
    /**
     * This method allows this class to call the method showStatusMsg
     * implemented by DFGUI
     */
    void setGUI(DFGUI g) {
        gui = g;
    }
    ...
    /**
     * returns "df" that is the name of the default DF. In fact, so far this
     * applet can be used only to interact with the default DF.
     */
    ...
    /**
     * According to the event generatated by the applet, this method performes
     * the needed actions.
     */
    ...
    /**
     * Refresh the gui of the applet. First of all makes a search for all agent
     * registered with the df, then updates the federate view, requesting the
     * parents to the df.
     */
    ...
    private void RegisterNewAgent(GuiEvent event) {
        ...
        } else //request the df to register an agent with another df.
        {
            try {
    ...
    private void DeregisterAgent(GuiEvent event) {
        ...
        } else //deregister the df from a parent
        {
            try {
