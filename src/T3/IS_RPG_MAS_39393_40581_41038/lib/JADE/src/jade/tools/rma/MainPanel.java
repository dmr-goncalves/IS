        ...
        if (paths != null) {
            ...
            for (int i = 0; i < paths.length; i++) {
                ...
                for (int j = 0; j < relCur.length; j++) {
                    if (relCur[j] instanceof AgentTree.AgentNode) //to display in the table only the agent.
                    {
                        agentPaths.add(paths[i]);
                    } else if (relCur[j] instanceof AgentTree.SuperContainer) {//show the APDescription in the TextArea
                    }
                }
            }
