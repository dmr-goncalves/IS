...
/**
 * This class implements a request protocol that can be used by an applet to
 * comunicate with a proxy agent.
 *
...
abstract class AppletRequestProto {
        ...
        try {
            ...
            if (ACLMessage.AGREE == (reply.getPerformative())) {
                ...
                if (ACLMessage.INFORM == inform.getPerformative()) {
                    handleInform(inform);
                } else if (ACLMessage.FAILURE == inform.getPerformative()) {
                    handleFailure(inform);
                } else {
                    handleOtherMessage(inform);
                }
            } else if (ACLMessage.INFORM == reply.getPerformative()) {
                handleInform(reply);
    ...
    /**
     * This method can be overriden by the programmer in order to handle the
     * received agree message.
     */
    ...
    /**
     * This method can be overriden by the programmer in order to handle the
     * notUnderstood message received.
     */
    protected abstract void handleNotUnderstood(ACLMessage msg);

    /**
     * This method can be overriden by the programmer in order to handle the
     * message not stated by the protocol eventually received.
     */
