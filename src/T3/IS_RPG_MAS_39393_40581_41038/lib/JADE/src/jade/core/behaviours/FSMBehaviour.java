 ...
 * <li> register one or more Behaviours as the final states of the FSM by
 * calling the method <code>registerLastState</code>;
 ...
 * <li> the method <code>registerDefaultTransition</code> is also useful in
 * order to register a default transition from a state to another state
 ...
 * @version $Date: 2008-04-15 11:09:57 +0200 (mar, 15 apr 2008) $ $Revision:
 * 6029 $
 *
 */
public class FSMBehaviour extends SerialBehaviour {
     ...
     * @return the Behaviour if any that was registered as the deregistered
     * state.
     */
    ...
    /**
     * Register a transition in the FSM defining the policy for children
     * scheduling of this <code>FSMBehaviour</code>.
     *
     ...
     * by the <code>onEnd()</code> method of the <code>Behaviour</code>
    ...
    /**
     * Register a transition in the FSM defining the policy for children
     * scheduling of this <code>FSMBehaviour</code>. When this transition is
     * fired the states indicated in the <code>toBeReset</code> parameter are
     * reset. This is particularly useful for transitions that lead to states
     ...
     * by the <code>onEnd()</code> method of the <code>Behaviour</code>
     * representing state s1.
     * @param toBeReset An array of strings including the names of the states to
     * be reset.
     * @see jade.core.behaviours.Behaviour#onEnd()
     */
     ...
     * scheduling of this <code>FSMBehaviour</code>. This transition will be
     * fired when state s1 terminates with an event that is not explicitly
     ...
     * scheduling of this <code>FSMBehaviour</code>. This transition will be
     * fired when state s1 terminates with an event that is not explicitly
     * associated to any transition. When this transition is fired the states
     ...
     *
     * @param source The name of the source state
     * @return <code>true</code> if a default transition exits from the given
     * source state. <code>false</code> otherwise.
     */
    ...
    /**
     * Retrieve the child behaviour associated to the FSM state with the given
     * name.
     *
     ...
     * state, or <code>null</code> if the given behaviour is not a child of this
     * FSM behaviour.
     */
    ...
    /**
     * Retrieve the exit value of the most recently executed child. This is also
     * the trigger value that selects the next FSM transition.
     *
    ...
    /**
     * This method schedules the next child to be executed. It checks whether
     * the current child is completed and, in this case, fires a suitable
     * transition (according to the termination event of the current child) and
     * schedules the child representing the new state.
     *
     ...
     * <code>onEnd()</code>) of the just executed child in the case this child
     * has completed (otherwise this parameter is meaningless)
     ...
     * This method acts as a sort of <code>GOTO</code> statement between states,
     * and replaces the currently active state without considering the trigger
     * event or whether a transition was registered. It should be used only to
     * handle exceptional conditions, if default transitions are not effective
     * enough.
     *
     * @param next The name of the state to jump to at the next FSM cheduling
     * quantum. If the FSM has no state with the given name, this method does
     * nothing.
     */
