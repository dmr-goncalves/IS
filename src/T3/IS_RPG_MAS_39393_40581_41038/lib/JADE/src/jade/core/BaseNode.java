...
/**
 * This class provides a partial implementation of the <code>Node</code>
 * interface. Concrete IMTPs will have to provide a full implementation of the
 * <code>Node</code> interface, possibly by subclassing this class.
 *
...
public abstract class BaseNode implements Node, Serializable {
    ...
    /**
     * Provides an IMTP independent implementation for the horizontal command
     * serving mechanism. IMTP dependent implementations of the accept() method
     ...
     * @throws ServiceException If the service the command belongs to is not
    ...
    /**
     * Serves an incoming vertical command, locally. This method is invoked if a
     * new <code>VerticalCommand</code> object is generated by a slice targetted
     * by a former <code>HorizontalCommand</code>, which happens if the
     * <code>Slice.serve()</code> yields a non-null result.
     *
     * This method makes it so that the newly created vertical command is handed
     * to the command processor to first pass through all incoming filters and
     * then to be dispatched to its proper incoming command sink.
     *
