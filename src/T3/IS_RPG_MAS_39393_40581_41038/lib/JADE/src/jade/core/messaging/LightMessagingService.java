...
/**
 * A minimal version of the JADE service to manage the message passing subsystem
 * installed on the platform. This class just supports direct ACL message
 * delivery, and relies on another one for any other feature (such as message
 * routing and MTP management).
 *
...
public class LightMessagingService extends BaseService
     ...
     * and process kernel-level commands.
    ...
    /**
     * This method is used internally by the platform in order to notify the
     * sender of a message that a failure was reported by the Message Transport
     * Service. Package scoped as it can be called by the MessageManager
     */
    ...
    /**
     * Inner mix-in class for this service: this class receives commands through
     * its <code>Filter</code> interface and serves them, coordinating with
     * remote parts of this service through the <code>Slice</code> interface
     * (that extends the <code>Service.Slice</code> interface).
     */
    private class ServiceComponent extends Filter implements Service.Slice {
        ...
        /**
         * Builds a new messaging service lightweight component, relying on a
         * remote slice for most operations.
         *
         */
