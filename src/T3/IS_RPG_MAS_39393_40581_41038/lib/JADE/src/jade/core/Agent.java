 ...
 * <li> <b> Scheduling and execution of multiple concurrent activities.
 * </b></li>
 * </ul>
 *
 ...
 * @version $Date: 2015-02-24 13:34:01 +0100 (mar, 24 feb 2015) $ $Revision:
 * 6740 $
 */
public class Agent implements Runnable, Serializable //#APIDOC_EXCLUDE_BEGIN
        ,
         TimerListener //#APIDOC_EXCLUDE_END
{

    private static final long serialVersionUID = 3487495895819000L;

    /**
     * Inner class Interrupted. This class is used to handle change state
     * requests that occur in particular situations such as when the agent
     * thread is blocked in the doWait() method.
     */
     ...
     * allows also to avoid a deadlock when: 1) A behaviour blocks for a very
     * short time --> A Timer is added to the TimerDispatcher 2) The Timer
     * immediately expires and the TimerDispatcher try to restart the behaviour
     * before the pair (b, t) is added to the pendingTimers of this agent.
     */
    ...
    /**
     * Notifies this agent that one of its behaviours has been restarted for
     * some reason. This method clears any timer associated with behaviour
     * object <code>b</code>, and it is unneeded by application level code. To
     * explicitly schedule behaviours, use <code>block()</code> and
     * <code>restart()</code> methods.
     *
     ...
     * termination (takeDown()) to allow it to clean-up properly. In this case
     * in fact a call to <code>doDelete()</code>, <code>doMove()</code>,
     * <code>doClone()</code> and <code>doSuspend()</code> should have no
     * effect.
     */
    ...
    //#MIDP_EXCLUDE_BEGIN
    /**
     * When set to false (default) all behaviour-related events (such as
     * ADDED_BEHAVIOUR or CHANGED_BEHAVIOUR_STATE) are not generated in order to
     * improve performances. These events in facts are very frequent.
     */
    ...
    //#MIDP_EXCLUDE_BEGIN
    /**
     * Developer can override this method to provide an alternative message
    ...
    /**
     * Declared transient because the container changes in case of agent
     * migration.
     */
     ...
     * <br>
     *
     * @return jade.wrapper.AgentContainer a controller for the container this
     * agent lives in.
     */
     ...
     * <p>
     * Take care that the arguments are transient and they do not migrate with
     * the agent neither are cloned with the agent!
     *
     * @return the array of arguments passed to this agent.
     * @see <a href=../../../tutorials/ArgsAndPropsPassing.htm>How to use
     * arguments or properties to configure your agent.</a>
     *
     */
    ...
    /**
     * This method returns <code>true</code> when this agent is restarting after
     * a crash. The restarting indication is automatically reset as soon as the
     ...
     *
     * @return An <code>AID</code> object, that can be used to contact the AMS
     * of this platform.
     */
     ...
     *
     * @return An <code>AID</code> object, that can be used to contact the
     * default DF of this platform.
     */
     ...
     * where the agent has been created, therefore it will never change during
     * the entire lifetime of the agent. In JADE the name of an agent by default
     * is composed by the concatenation (using '@') of the agent local name and
     * the Home Agent Platform name
     *
     * @return A <code>String</code> containing the name of the home agent
     * platform (e.g. <em>myComputerName:1099/JADE</em>).
     */
     ...
     *
     * @return An <code>Agent ID</code> object, containing the complete agent
     * GUID, addresses and resolvers.
     */
    ...
    /**
     * Method to build a complete agent GUID belonging to the same platform of
     * the current agent
     *
     ...
     * called by the container when a new MTP is activated in the platform (in
     * the local container - installMTP() - or in a remote container -
     ...
     * It is called by the container when a new MTP is deactivated in the
     * platform (in the local container - uninstallMTP() - or in a remote
     * container - updateRoutingTable()) to keep the Agent AID updated.
     */
     ...
     *
     * @return A <code>Location</code> object, describing the location where
     ...
     * termination. We have already called doDelete on the thread which would
     * have issued an interrupt on it. However, it still may decide not to exit.
     * So we will wait no longer than 5 seconds for it to exit and we do not
     * care of this zombie agent. FIXME: we must further isolate container and
     * agents, for instance by using custom class loader and dynamic proxies and
     * JDK 1.3. FIXME: the timeout value should be got by Profile
     */
    public void join() {
        //#MIDP_EXCLUDE_BEGIN
        try {
     ...
     * Passing 0 means unlimited message queue. When the number of buffered
     * messages exceeds this value, older messages are discarded according to a
     * <b><em>FIFO</em></b> replacement policy.
    ...
    /**
     * This method retrieves the current length of the message queue of this
     * agent.
     *
     ...
     *
     * @return The actual size of the message queue (i.e. the max number of
     * messages that can be stored into the queue)
    ...
    //#APIDOC_EXCLUDE_BEGIN
    /**
     * Read current agent state. This method can be used to query an agent for
    ...
    /**
     * This is only called by the NotificationService to provide the
     * Introspector agent with a snapshot of the behaviours currently loaded in
     * the agent
     */
     ...
     * support agent mobility and is called either by the Agent Platform or by
     * the agent itself to start a migration process. It should be noted that
     * this method just changes the agent state to <code>AP_TRANSIT</code>. The
     * actual migration takes place asynchronously.
     * <br>
     * <b>NOT available in MIDP</b>
     * <br>
     *
     ...
     * support agent mobility and is called either by the Agent Platform or by
     * the agent itself to start a clonation process. It should be noted that
     * this method just changes the agent state to <code>AP_COPY</code>. The
     * actual clonation takes place asynchronously.
     * <br>
     * <b>NOT available in MIDP</b>
     * <br>
     *
     * @param destination The <code>Location</code> where the copy agent will
     * start.
     ...
     * to <em>suspended</em> within Agent Platform Life Cycle; the original
     * agent state is saved and will be restored by a <code>doActivate()</code>
     * call. This method can be called from the Agent Platform or from the agent
     * itself and stops all agent activities. Incoming messages for a suspended
     * agent are buffered by the Agent Platform and are delivered as soon as the
     * agent resumes. Calling <code>doSuspend()</code> on a suspended agent has
     * no effect.
     * <br>
     * <b>NOT available in MIDP</b>
     * <br>
     ...
     * <em>active</em> or <em>waiting</em> (whichever state the agent was in
     * when <code>doSuspend()</code> was called) within Agent Platform Life
     * Cycle. This method is called from the Agent Platform and resumes agent
     * execution. Calling <code>doActivate()</code> when the agent is not
     ...
     * within Agent Platform Life Cycle. This method has only effect when called
     * by the agent thread and causes the agent to block, stopping all its
     * activities until a message arrives or the <code>doWake()</code> method is
     * called.
     *
    ...
    /**
     * Make a state transition from <em>active</em> to <em>waiting</em>
     * within Agent Platform Life Cycle. This method adds a timeout to the other
     ...
     * within Agent Platform Life Cycle. This method is called from Agent
     * Platform and resumes agent execution. Calling <code>doWake()</code> when
     * an agent is not waiting has no effect.
     *
     * @see jade.core.Agent#doWait()
     */
    public void doWake() {
     ...
     * or <em>waiting</em> to <em>deleted</em> state within Agent Platform Life
     * Cycle, thereby destroying the agent. This method can be called either
     * from the Agent Platform or from the agent itself. Calling
     * <code>doDelete()</code> on an already deleted agent has no effect.
     */
     ...
     * connection. Of course, the whole agent must be serializable in order to
    ...
    /**
     * Read a previously saved agent from an input stream and restarts it under
     * its former name. This method can realize some sort of mobility through
     * time, where an agent is saved, then destroyed and then restarted from the
     * saved copy.
     * <br>
     * <b>NOT available in MIDP</b>
     * <br>
     ...
     * <em>not</em> closed on exit.
     * @exception IOException Thrown if some I/O error occurs during stream
     * reading.
    ...
    /**
     * Read a previously saved agent from an input stream and restarts it under
     * a different name. This method can realize agent cloning through streams,
     * where an agent is saved, then an exact copy of it is restarted as a
     * completely separated agent, with the same state but with different
     ...
     * @param s The stream the agent will be read from. The stream is
     * <em>not</em> closed on exit.
     * @param agentName The name of the new agent, copy of the saved original
     * one.
     * @exception IOException Thrown if some I/O error occurs during stream
     * reading.
     * @see jade.core.Agent#write(OutputStream s)
     ...
     */
    /**
     * This method reads a previously saved agent, replacing the current state
     * of this agent with the one previously saved. The stream must contain the
     * saved state of <b>the same agent</b> that it is trying to restore itself;
     * that is, <em>both</em> the Java object
    ...
    /**
     * This method should not be used by application code. Use the same-named
     * method of <code>jade.wrapper.Agent</code> instead.
     * <br>
     * <b>NOT available in MIDP</b>
     * <br>
     *
     * @see jade.wrapper.AgentController#putO2AObject(Object o, boolean
     * blocking)
     */
    public void putO2AObject(Object o, boolean blocking) throws InterruptedException {
        ...
        if (blocking) // Sleep on the condition. This method is synchronized on the condvar
        {
            cond.waitOn();
        }
    }
     ...
     * object-to-agent communication queue. In order for this method to work,
     * the agent must have declared its will to accept objects from other
     * software components running within its JVM. This can be achieved by
     * calling the <code>jade.core.Agent.setEnabledO2ACommunication()</code>
     * method. If the retrieved object was originally inserted by an external
     ...
     * <br>
     *
     * @return the first object in the queue, or <code>null</code> if the queue
     * is empty.
     * @see jade.wrapper.AgentController#putO2AObject(Object o, boolean
     * blocking)
     ...
     * communication, that is, whether the agent accepts to communicate with
     * other non-JADE components living within the same JVM.
     * <br>
     * <b>NOT available in MIDP</b>
     * <br>
     *
     ...
     * parameter specifiies the maximum number of Java objects that will be
     * queued. If the passed value is 0, no maximum limit is set up for the
     * queue.
     *
     * @see jade.wrapper.AgentController#putO2AObject(Object o, boolean
     * blocking)
    ...
    /**
     * Sets the behaviour responsible for managing objects passed to the agent
     * by means of the Object-To-Agent (O2A) communication mechanism. If the O2A
     * manager behaviour is set, whenever an object is inserted in the O2A queue
     * by means of the <code>putO2AObject()</code> method, only the manager is
     * waken up. This improves the efficiency since all behaviours not
     * interested in O2A communication remain sleeping. <br>
     ...
     * channel; in order to correctly run, the behaviour must be added to the
     * agent by means of the 
     ...
     *
     * @param theInterface The O2A interface the implementation is registered
     * for.
     * @param implementation The object providing an implementation for the
     * given O2A interface.
     */
    ...
    //#APIDOC_EXCLUDE_BEGIN
    /**
     * This method is the main body of every agent. It provides startup and
     * cleanup hooks for application programmers to put their specific code
     * into.
     *
     ...
     * behaviour. When this method is called the agent has been already
     * registered with the Agent Platform <b>AMS</b>
     * and is able to send and receive messages. However, the agent execution
     * model is still sequential and no behaviour scheduling is active yet.
     *
     * This method can be used for ordinary startup tasks such as
     * <b>DF</b> registration, but is essential to add at least a
     * <code>Behaviour</code> object to the agent, in order for it to be able to
     * do anything.
     *
     ...
     * behaviour. When this method is called the agent has not deregistered
     * itself with the Agent Platform <b>AMS</b>
     * and is still able to exchange messages with other agents. However, no
     * behaviour scheduling is active anymore and the Agent Platform Life Cycle
     * state is already set to
     * <em>deleted</em>.
     *
     * This method can be used for ordinary cleanup tasks such as
     * <b>DF</b> deregistration, but explicit removal of all agent behaviours is
     * not needed.
     */
    ...
    //#MIDP_EXCLUDE_BEGIN
    /**
     * This empty placeholder shall be overridden by user defined agents to
     * execute some actions before the original agent instance on the source
     * container is stopped (e.g. releasing local resources such as a GUI).<br>
     ...
     * destination container Therefore setting the value of a class field in
     * this method will have no impact on the moved agent instance. Such
     * parameters must indeed be set <b>before</b> the <code>doMove()</code>
     * method is called.
     * <br>
     * <b>NOT available in MIDP</b>
     * <br>
     ...
     * to execute some actions before copying an agent to another agent
     * container.
     * <br>
     * <b>NOT available in MIDP</b>
     * <br>
     ...
     * executed concurrently with all the others, using a cooperative round
     * robin scheduling. This method is typically called from an agent
     * <code>setup()</code> to fire off some initial behaviour, but can also be
     * used to spawn new behaviours dynamically.
     *
    ...
    /**
     * This method removes a given behaviour from the agent. This method is
     * called automatically when a top level behaviour terminates, but can also
     * be called from a behaviour to terminate itself or some other behaviour.
     *
    ...
    /**
     * Send an <b>ACL</b> message to another agent. This methods sends a message
     * to the agent specified in <code>:receiver</code> message field (more than
     * one agent can be specified as message receiver).
     *
    ...
    /**
     * Receives an <b>ACL</b> message from the agent message queue. This method
     * is non-blocking and returns the first message in the queue, if any.
     * Therefore, polling and busy waiting is required to wait for the next
     ...
     * Therefore, polling and busy waiting is required to wait for a specific
     * kind of message using this method.
     *
    ...
    /**
     * Receives an <b>ACL</b> message from the agent message queue. This method
     * is blocking and suspends the whole agent until a message is available in
     * the queue.
     *
    ...
    /**
     * Receives an <b>ACL</b> message from the agent message queue, waiting at
     * most a specified amount of time.
     *
     * @param millis The maximum amount of time to wait for the message.
     * @return A new ACL message, or <code>null</code> if the specified amount
     * of time passes without any message reception.
     */
    ...
    /**
     * Receives an <b>ACL</b> message matching a given message template. This
     * method is blocking and suspends the whole agent until a message is
     * available in the queue.
     *
     * @param pattern A message template to match received messages against.
     * @return A new ACL message matching the given template, blocking until
    ...
    /**
     * Receives an <b>ACL</b> message matching a given message template, waiting
     * at most a specified time.
     *
     * @param pattern A message template to match received messages against.
    ...
    /**
     * Puts a received <b>ACL</b> message back into the message queue. This
     * method can be used from an agent behaviour when it realizes it read a
     * message of interest for some other behaviour. The message is put in front
     * of the message queue, so it will be the first returned by a
     * <code>receive()</code> call.
     *
     * @see jade.core.Agent#receive()
     */
     ...
     * until just before its setup() method is called. When this method returns,
     * the target agent is registered with the AMS and the JADE platform is
     * aware of it.
     */
    ...
    /**
     * Put a received message into the agent message queue. The message is put
     * at the back end of the queue. This method is called by JADE runtime
     * system when a message arrives, but can also be used by an agent, and is
     * just the same as sending a message to oneself (though slightly faster).
     *
    ...
    /**
     * This method is used to interrupt the agent's thread. In J2SE/PJAVA it
     * just calls myThread.interrupt(). In MIDP, where interrupt() is not
     * supported the thread interruption is simulated as described below. The
     * agent thread can be in one of the following three states: 1) Running a
     * behaviour. 2) Sleeping on msgQueue due to a doWait() 3) Sleeping on
     * myScheduler due to a schedule() with no active behaviours Note that in
     * MIDP the suspended state is not supported The idea is: set the
     * 'isInterrupted' flag, then wake up the thread wherever it may be
     */
     ...
     * interruption is used to "interrupt" the agent's thread, we must check
     * whether the simulated interruption happened just before and after going
     * to sleep.
     */
