...
public final class ReceiverBehaviour extends Behaviour {
     ...
     * obtain an <code>ACLMessage</code> object from an <code>Handle</code>, but
     * no message was received within a specified timeout.
     *
     ...
     * message was received so far, but the time limit is not yet reached.
     *
     * @see jade.core.behaviours.ReceiverBehaviour.Handle#getMessage()
     */
     ...
     * This interface is used to create a <code>ReceiverBehaviour</code> object
     * to receive an ACL message within a user specified time limit. When the
     * user tries to read the message represented by the handle, either gets it
     * or gets an exception.
     *
    ...
    public static interface Handle {
        ...
        /**
         * Tries to retrieve the <code>ACLMessage</code> object represented by
         * this handle.
         *
         * @return The ACL message, received by the associated
         * <code>ReceiverBehaviour</code>, if any.
         * @exception TimedOut If the associated <code>ReceiverBehaviour</code>
         * did not receive a suitable ACL message within the time limit.
         * @exception NotYetReady If the associated
         * <code>ReceiverBehaviour</code> is still waiting for a suitable ACL
         * message to arrive.
     ...
     * <code>Handle</code> object, which can be used to retrieve an ACL message
     * out of a <code>ReceiverBehaviour</code> object.
     *
    ...
    /**
     * This constructor creates a <code>ReceiverBehaviour</code> object that
     * ends as soon as an ACL message matching a given
     * <code>MessageTemplate</code> arrives or the passed <code>millis<code> timeout expires.
     ...
     * (infinite time if <code>millis < 1</code>). When calling this
     * constructor, a suitable <code>Handle</code> must be created and passed to
     * it. When this behaviour ends, some other behaviour will try to get the
     * ACL message out of the handle, and an exception will be thrown in case of
     * a time out. The following example code explains this:
     *
     * <code><pre>
     ...
     * <code>millis</code> milliseconds (infinite time if
     * <code>millis < 1</code>. When calling this constructor, a suitable
     * <code>Handle</code> must be created and passed to it.
     *
     * @param a The agent this behaviour belongs to.
     * @param h An <em>Handle</em> representing the message to receive.
     * @param millis The maximum amount of time to wait for the message, in
    ...
    /**
     * Actual behaviour implementation. This method receives a suitable ACL
     * message and copies it into the message provided by the behaviour creator.
     * It blocks the current behaviour if no suitable message is available.
     */
    public void action() {
        ACLMessage msg = null;
        if (template == null) {
     ...
     * <code>ACLMessage</code> with the same <code>ReceiverBehaviour</code>
     ...
     * @exception NotYetReady if the message is not yet arrived and the timeout
     * is not yet expired.
     *
     */
    public ACLMessage getMessage() throws TimedOut, NotYetReady {
