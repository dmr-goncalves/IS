...
/**
 * The FaultRecovery service allows recovering a platform after a fault and a
 * successive restart of the main container.
 *
 * On a Main container this service keeps track of platform nodes in a
 * persistent storage. When the platform shuts down the persistent storage is
 * cleared. At bootstrap time the service gets all nodes from the persistent
 * storage (there are nodes only if the main container is restarting after a
 * crash) and notifies them (if still alive) about the recovery.
 *
 ...
 * ServiceManager) and issues a Service.REATTACHED incoming V-Command. The
 * FaultRecovery service filter intercepts this command and re-adds all agents
...
public class FaultRecoveryService extends BaseService {
    ...
    /**
     * Inner class MainCommandIncomingFilter. This filter is installed on a Main
     * Container and intercepts the NEW_NODE and DEAD_NODE V-Commands
     */
    ...
    /**
     * Inner class MainCommandOutgoingFilter. This filter is installed on a Main
     * Container and intercepts the NODE_UNREACHABLE and NODE_REACHABLE
     * VCommands of the NodeMonitoringService and the ORPHAN_NODE VCommand of
     * the UDPNodeMonitoringService.
     */
    ...
    /**
     * Inner class ContainerCommandIncomingFilter. This filter is installed on a
     * peripheral Container and intercepts the REATTACHED V-Commands
     */
