...
/**
 * Helper class that allows agents to exploit the features of the
...
public interface AgentReplicationHelper extends ServiceHelper {
     ...
     * getHelper() method of the Agent class to retrieve the
     ...
     * hot resources. That is messages directed to the virtual agent are
     * delivered indifferently to one of the available replica
     */
    static final int HOT_REPLICATION = 1;
    /**
     * The constant indicating that replicas of a virtual agent are treated as
     * cold resources. That is messages directed to the virtual agent are always
     * delivered to the master replica. Other replica(s) are only there for
     * backup purposes
     */
    static final int COLD_REPLICATION = 2;

    /**
     * ACL Message user defined parameter key specifying the local name of the
     * virtual agent the message was sent to. This is automatically set by the
     * AgentReplicationService when resolving a virtual agent receiver to one of
     * its implementation replicas
     */
    static final String VIRTUAL_RECEIVER = "JADE-virtual-receiver";

    /**
     * The interface to be implemented by a replicated agent for the master
     * replica to be notified about replica addition/removal and master replica
     * changes. If a replicated agent does not implement this interface such
     * events will not be notified.
     */
    ...
    /**
     * Define a virtual agent and sets the agent that invokes this method as its
     * master replica.
     *
     * @param virtualName The name of the virtual agent.
     * @param replicationMode Indicates whether replicas of the defined virtual
     * agent must be treated as hot or cold resources.
     ...
     * creation by means of the replicaAdded() and replicaCreationFailed()
     * methods of the Listener interface.
     *
     * @param replicaName The name of the new replica agent
     * @param where The location where the new replica agent must be cloned
     * @throws ServiceException If the agent invoking this method did not define
     * a virtual agent yet, it is not the master replica or an error occurs.
     */
    void createReplica(String replicaName, Location where) throws ServiceException;

    /**
     * Returns the AID of the virtual agent previously defined by means of the
     * makeVirtual() method or null if no virtual agent has been defined.
     *
     * @return The AID of the virtual agent previously defined by means of the
     * makeVirtual() method or null if no virtual agent has been defined.
     */
    AID getVirtualAid();
