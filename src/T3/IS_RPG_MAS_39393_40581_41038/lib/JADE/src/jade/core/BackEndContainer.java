...
public class BackEndContainer extends AgentContainerImpl implements BackEnd {
    ...
    /////////////////////////////////////
    /**
     * A new agent has just started on the FrontEnd. Adjust the agent name
     * taking into account wild-cards. Issue an INFORM_CREATED vertical command.
     *
    ...
    /**
     * An agent has just died on the FrontEnd. Remove its image and notify the
     * Main
     */
    ...
    /**
     * An agent on the FrontEnd has sent a message. Note that the
     * NotFoundException here is referred to the sender and indicates an
     * inconsistency between the FrontEnd and the BackEnd
     */
     ...
     * by a thread that is serving a message sent by an agent in the FrontEnd
     * too, nothing is done as the dispatch has already taken place in the
     * FrontEnd (see messageOut()).
     */
    ...
    /**
     * This method is re-defined to avoid NullPointerException. In fact a search
     * in the LADT would be done for the agent to be debugged, but the LADT is
     * obviously empty.
     */
    ...
    /**
     * Start the front-end synchronization process. NOTE that when this method
     * is called it may not be possible to send commands to the FE. If a
     * bi-connection dispatcher is used in fact, the INP connection is not yet
     * ready at this time. However we can't just send a SYNCH command and rely
     * on the store-and-forward mechanism of the FrontEndStub since we need to
     * know when the SYNCH command is completely served. --> We start a
     * separated Thread that tries sending the SYNCH command until success.
     */
