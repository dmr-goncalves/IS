 ...
 * @version $Date: 2015-12-21 12:24:43 +0100 (lun, 21 dic 2015) $ $Revision:
 * 6778 $
 *
 */
class AgentContainerImpl implements AgentContainer, AgentToolkit {
    ...
    /**
     * Return a proxy that allows making requests to the local container as if
     * they were received from the main. This allows dealing uniformly with
     * local and remote requests. Local requests occurs at bootstrap and
     * following calls to the in-process interface.
     */
    ...
    /**
     * Issue an INFORM_CREATED vertical command. Note that the Principal, if
     * any, is that of the owner of the newly born agent, while the Credentials,
     * if any, are a set additional initial credentials to be attached to the
     * newly born agent. The SecurityService, if active, will create a new
     * Principal for the newly born agent and will initialize its credentials as
     * the union of the initial credential and the ownership certificate.
     */
    ...
    public void handleChangedAgentPrincipal(AID agentID, JADEPrincipal oldPrincipal, Credentials creds) {
         ...
         * myPlatform.changedAgentPrincipal(agentID, certs); } catch
         * (IMTPException re) { re.printStackTrace(); } catch (NotFoundException
