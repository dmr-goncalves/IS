...
public abstract class Profile {
    ...
    /**
     * This constant is the name of the property whose value contains a boolean
     * indicating if this is the Main Container or a peripheral container.
     */
    ...
    /**
     * This constant is the name of the property whose value is the name (or the
     * IP address) of the network host where the JADE Main Container is running.
     */
    public static final String MAIN_HOST = "host";

    /**
     * This constant is the name of the property whose value contains an integer
     * representing the port number where the Main Container is listening for
     * container registrations.
     */
    public static final String MAIN_PORT = "port";

    /**
     * This constant is the name of the property whose Boolean value tells
     * whether to activate the automatic main container detection mechanism. By
     * means of this mechanism a peripheral container is able to automatically
     * detect the main container host and port at startup time. The mechanism is
     * based on IP multicast communication and must be activated on both the
     * main container, that publishes its host and port on a given multicast
     * address (by default 239.255.10.99, port 1199), and on peripheral
     * containers. The default for this option is <code>true</code> on Main
     * Containers and <code>false</code> on peripheral containers
     */
    public static final String DETECT_MAIN = "detect-main";

    /**
     * This constant is the name of the property whose value (true or false)
     * instructs JADE to try to use host names instead of IP addresses in
     * intra-platform communication. Default is false (IP addresses are used
     * preferentially)
     */
    public static final String PRIVILEDGE_LOGICAL_NAME = "priviledge-logical-name";

    /**
     * This constant is the name of the property whose value contains the host
     * name the container must bind on. The host name must refer to the local
     * machine, and is generally needed only when multiple network interfaces
     * are present or a non-default name is desired.
     */
    public static final String LOCAL_HOST = "local-host";

    /**
     * This constant is the name of the TCP port the container node must listen
     * to for incoming IMTP messages.
     */
    public static final String LOCAL_PORT = "local-port";

    /**
     * This constant is the name or address of the host that other containers
     * will have to use to contact this container
     */
    public static final String EXPORT_HOST = "export-host";

    /**
     * This constant is the port that other containers will have to use to
    ...
    /**
     * This constant is the name of the property whose Boolean value tells
     * whether a local Service Manager is exported by this container (only when
     * using JADE support for fault-tolerant platform configurations).
     */
    public static final String LOCAL_SERVICE_MANAGER = "backupmain";

    /**
     * This constant is the name of the property whose Boolean value tells
     * whether startup options should be dumped. Default is false
     */
    public static final String DUMP_OPTIONS = "dump-options";

    /**
     * This constant, when set to <code>true</code> tells the JADE runtime that
     * it will be executed in an environment with no display available.
     */
    ...
    //#APIDOC_EXCLUDE_END
    /**
     * This constant is the name of the property whose value contains the unique
     * platform ID of a JADE platform. Agent GUIDs in JADE are made by a
     * platform-unique nickname, the '@' character and the platform ID.
     */
    public static final String PLATFORM_ID = "platform-id";

    /**
     * This constant is the name of the property whose value contains the user
     * authentication type to be used to login to the JADE platform.
     */
    public static final String USERAUTH_KEY = "userauth-key";

    /**
     * This constant is the name of the property whose value contains the list
     * of agents that have to be launched at bootstrap time
     */
    public static final String AGENTS = "agents";

    /**
     * This constants is the name of the property whose value contains the list
     * of kernel-level services that have to be launched at bootstrap time
     */
    public static final String SERVICES = "services";

    /**
     * This constant is the name of the property whose value contains the list
     * of addresses through which the platform <i>Service Manager</i> can be
     * reached.
     */
    public static final String REMOTE_SERVICE_MANAGER_ADDRESSES = "smaddrs";

    /**
     * This constant is the key of the property whose value contains the list of
     * MTPs that have to be launched at bootstrap time. This list must be
     * retrieved via the <code>getSpecifiers(MTPS)<code>
     * method.
     */
    ...
    /**
     * This constant is the key of the property whose value identifies the IMTP
     * Manager to be created by ProfileImpl
     *
     */
    public static final String IMTP = "imtp";

    /**
     * This constant is the key of the property whose value contains the desired
     * name of the container. If this container name exists already, then a
     * default name is assigned by the platform. The name of the main-container
     * is always assigned by the platform and cannot be changed.
     *
     */
    public static final String CONTAINER_NAME = "container-name";

    /**
     * This constant is the key of the property whose value will substitute the
     * %A wildcard in names of all agents that will start in the local
     * container.
     */
    ...
    /**
     * This constant is the key of the property whose value contains the list of
     * ACLCODECSs that have to be launched at bootstrap time. This list must be
     * retrieved via the <code>getSpecifiers(ACLCODECS)<code>
     * method.
     */
     ...
     * indicates whether or not this platform accepts foreign agents i.e. agents
     * whose names are not of the form <local-name>@<platform-name>.
     */
    ...
    /**
     * This constant is the key of the property whose value contains the name of
     * the directory where all the files generated by JADE should be put. The
    ...
    /**
     * This constant is the key of the property whose value contains the version
     * of IP address to use.
     */
    ...
    /**
     * Obtain a reference to the platform <i>Service Manager</i>, with which
     * kernel-level services can be added and removed.
     *
     * @return A <code>ServiceManager</code> object, representing the platform
     * service manager.
     */
    protected abstract ServiceManager getServiceManager() throws ProfileException;

    /**
     * Obtain a reference to the platform <i>Service Finder</i>, with which
     * kernel-level services can be looked up.
     *
     * @return A <code>ServiceFinder</code> object, representing the platform
     * service manager.
     */
    protected abstract ServiceFinder getServiceFinder() throws ProfileException;

    /**
     * Obtain a reference to the container <i>Command Processor</i>, which
     * manages kernel-level commands dispatching them to the proper platform
     * services.
     *
     * @return A <code>ServiceManager</code> object, representing the platform
     * service manager.
     ...
     * object, i.e. without internal initializations automatically performed by
     * the Profile class.
     */
    ...
    /**
     * Retrieve a String value from the configuration properties. If no
     * parameter corresponding to the specified key is found, return the
     * provided default.
     *
     ...
     * configuration properties.
     * @param aDefault The value to return when there is no property set for the
     * given key, or its value cannot be converted to a boolean value.
     */
    public abstract boolean getBooleanProperty(String key, boolean aDefault);

    /**
     * Retrieve a list of Specifiers from the configuration properties. Agents,
     * MTPs and other items are specified among the configuration properties in
     * this way. If no list of Specifiers corresponding to the specified key is
    ...
    /**
     * Provides a boolean indication about whether or not the container using
     * this Profile is a Main Container.
     *
     * @return <code>true</code> if the container using this Profile is a Main
    ...
    /**
     * Provides a boolean indication about whether or not the container using
     * this Profile is the Master Main Container.
     *
     * @return <code>true</code> if the container using this Profile is the
     * Master Main Container. <code>false</code> otherwise
     */
    ...
    /**
     * Provides a boolean indication about whether or not the container using
     * this Profile is a Backup Main Container.
     *
     * @return <code>true</code> if the container using this Profile is a Backup
     * Main Container. <code>false</code> otherwise
     */
    ...
    //#MIDP_EXCLUDE_BEGIN
    /**
     * Check whether or not a given host name or address corresponds to a local
     * network interface
     */
    ...
    /**
     * Compares two host names/addresses regardless of whether they include
     * domain or not.
     */
    ...
    /**
     * Compares two stringified transport addresses. The provided IMTPManager
     * must be able to deal with the protocols (e.g. http) the two transport
     * address refer to.
     */
