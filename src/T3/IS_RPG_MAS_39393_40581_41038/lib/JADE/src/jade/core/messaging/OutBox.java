...
/**
 * Object to maintain message to send and to preserve the order for sending.
 *
...
class OutBox {
     ...
     * receiver. If a Box for the indicated receiver is not yet present, a new
     * one is created. This method is executed by an agent's thread requesting
     * to deliver a new message.
     */
     ...
     * receiver. This method is executed by the TimerDispatcher Thread when a
     * retransmission timer expires. Therefore a Box of messages for the
     * indicated receiver must already exist. Moreover the busy flag of this Box
     * must be reset to allow deliverers to handle messages in it
     *
     * synchronized void addFirst(PendingMsg pm){ Box b = (Box)
     * messagesByReceiver.get(pm.getReceiver()); b.addFirst(pm);
     * b.setBusy(false); // Wakes up all deliverers notifyAll(); }
     */
    /**
     * Get the first message for the first idle (i.e. not busy) receiver. This
     * is executed by a Deliverer thread just before delivering a message.
     */
     ...
     *
     * @return null if all receivers are currently busy This method does not
     * need to be synchronized as it is only executed inside a synchronized
     * block.
     */
    ...
    /**
     * Some messages for the receiver receiverID have been served If the Box of
     * messages for that receiver is now empty --> remove it. Otherwise just
     * mark it as idle (not busy).
     */
    ...
    /**
     * The method decreases the value of size and, eventually, set to false the
     * value of overMaxSize.
     * <p>
     * This method should have been declared synchronized. For possible better
     * peformance, it is not declared synchronized because it is a private
     * method and it is called only by the method get() which is already
     * synchronized.
     *
    ...
    /**
     * This class represents a Box of messages to be delivered to a single
     * receiver
     */
    private class Box {
