...
public class ThreadedBehaviourFactory {
     ...
     * Adding the wrapper behaviour to an agent results in executing
     * <code>b</code> in a dedicated Java Therad.
     */
    ...
    /**
     * Interrupt all threaded behaviours managed by this
     ...
     * threaded behaviour instead of getThread().interrupt() because i) the
     * latter may have no effect if called just after the threaded behaviour
     * suspended itself and ii) the threaded behaviour may be suspended and in
     * this case its Thread is null.
     *
    ...
    /**
     * Suspend a threaded behaviour. This method has only effect if called by
     * the threaded behaviour itself and has the effect of releasing its
     * dedicated Java Thread. This can later be restored by means of the
    ...
    /**
     * Resume a threaded behaviour. Assign a new Java Thread to a threaded
     * behaviour that is currently suspended.
     */
    ...
    /**
     * @return the Thread dedicated to the execution of the Behaviour
     * <code>b</code>
     */
    ...
    public class ThreadedBehaviourWrapper extends Behaviour implements Runnable {
         ...
         * <code>parent</code> member variable of the wrapped behaviour must
         * point to the DummyParentBehaviour --> From the wrapped behaviour
         ...
         * ThreadedBehaviourWrapped is directly added to the agent Scheduler and
         * a message is received) to the wrapped threaded behaviour.
         */
        public void restart() {
            myBehaviour.restart();
        }
         ...
         * and the latter, or an ancestor, is blocked/restarted) to the wrapped
         * threaded behaviour. If the event is a restart, also notify the
         * dedicated thread.
         */
         ...
         * getThread().interrupt() because the latter may have no effect if
         * called just after the threaded behaviour suspended itself
         */
    ...
    private class DummyParentBehaviour extends CompositeBehaviour {
        ...
        /**
         * Redefine the root() method so that both the DummyParentBehaviour and
         * the ThreadedBehaviourWrapper are invisible in the behaviours
         * hierarchy
         */
