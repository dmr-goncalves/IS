...
/**
 *
 * The JADE service to manage the persistent storage of undelivered ACL messages
...
public class PersistentDeliveryService extends BaseService {
    ...
    /**
     * This constant is the name of the property whose value contains the name
     * of the application-specific class that will be used by the
     ...
     * semicolon separated names of the nodes where messages can be stored. By
     * default all nodes are considered. This implies that whenever the delivery
     * of a message fails all slices are asked to see if they have a filer that
     * must store the message. Similarly whenever a new agent is created all
     * slices are asked to see if they have messages to flush. When the platform
     * is composed by 100 or more containers (e.g. when there are mobile
     * terminals running split containers) this may be very un-efficient -->
     * using this option to limit the number of slices to be asked is highly
     * recommended
     */
     ...
     * representing how often (in milliseconds) the PersistentDeliveryService
     * will try to send again previously undelivered ACL messages which have
     * been buffered.
     */
    ...
    /**
     * This constant is the name of the property whose value contains the
     * storage method used to persist undelivered ACL messages by the
     * PersistentDeliveryService on the local container. The supported values
     * for this parameter are:
     * <ul>
     * <li><b>file</b> - A directory tree on the local filesystem is used.</li>
     * </ul>
     * If this property is not specified undelivered ACL messages are kept in
     * memory and not persisted at all.
     */
    public static final String PERSISTENT_DELIVERY_STORAGEMETHOD = "persistent-delivery-storagemethod";

    /**
     * This constant is the name of the property whose value contains the root
     * of the directory tree that is used to persist undelivered ACL messages
     * when the <i>file</i> storage method is selected.
     */
    ...
    /**
     * Outgoing command FILTER. Processes the NOTIFY_FAILURE command
     */
    ...
    /**
     * Incoming command FILTER. Processes the INFORM_CREATED command. Note that
     * we do this in the postProcess() method so that we are sure the newly born
     * agent is already in the GADT.
     */
    ...
    private class ServiceComponent implements Service.Slice {
        ...
        /**
         * This is called following a message delivery failure to check whether
         * or not the message must be stored.
         */
     ...
     * PersistentDeliveryFilter. Note that getting the MessagingService
     * (required to instantiate the PersistentDeliveryManager) cannot be done in
     * the init() method since at that time the MessagingService may not be
     * installed yet.
     */
    ...
    /**
     * Requests all slices to flush the stored messages for a newly born target
     * agent. Do it in a separated thread since this may take time. This happens
     * on the main container only.
     */
