...
public class OutgoingEncodingFilter extends Filter {
     ...
     * representation and adjusting Envelope fields: 1) If the receiver lives in
     * the local container --> Do not encode (to speed up performances) -->
     * Don't touch the envelope 2) If the receiver lives in a remote container
     * --> Encode using the specified representation or "LEAP" if no
     * representation is specified --> If an envelope is present adjust its
     * fields 3) If the receiver lives in a remote platform --> Encode using the
     * specified representation or "String" if no representation is specified
    ...
    /**
     * Encodes an ACL message according to the acl-representation described in
     * the envelope. If there is no explicit acl-representation in the envelope,
