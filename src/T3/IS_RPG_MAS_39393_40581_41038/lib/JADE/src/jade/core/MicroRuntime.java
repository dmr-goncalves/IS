 ...
 * container. Though JADE supports split containers on all Java editions, the
 * split container deployment is better suited for small, resource constrained
 * devices (MIDP and PJava).
 *
...
public class MicroRuntime {
    ...
    /**
     * The configuration property key that maps to the list of agents that have
     * to be activated at boostrap.
     */
    public static final String AGENTS_KEY = "agents";

    /**
     * The configuration property key that maps to the list of services that
     ...
     * is the base for using a split container in a JADE platform with some form
     * of security as it is automatically propagated to the back-end. For
     * instance to connect a split-container to a platform using JADE-S
     * Front-End: In the startup peroperties specify - owner =
     * <username>:<password>
     * Back-End: In the leap property file of the JICPMediatorManager that will
     * host the BackEnd specify - services = ....<security services>... -
    ...
    /**
     * The configuration property key that maps to the protocol that must be
     * used to connect to the JADE mediator. Possible values are:
     * <code>socket, ssl, http, https</code>
     */
    public static final String PROTO_KEY = "proto";

    /**
     * The configuration property key that specifies how much time by default a
     * command that is being delivered during a temporary disconnection must be
     * stored waiting for the FE to reconnect. After that time the command fails
     * (in case of a message delivery command, a FAILURE is sent back to the
     * sender).<br>
    ...
    /**
     * Start up the JADE runtime. This method launches a JADE Front End
     * container. Since JADE supports only one container in the split-container
     * deployment, if a Front End is already running this method does nothing.
     *
     * @param p A property bag, containing name-value pairs used to configure
     * the container during boot.
     * @param r A <code>Runnable</code> object, whose <code>run()</code> method
     * will be executed just after container termination.
     */
    ...
    /**
     * Tells whether a JADE Front End container is currently running within this
     * JVM.
     *
     * @return If the JADE runtime is currently running, <code>true</code> is
     * returned. Otherwise, the method returns <code>false</code>.
     */
     ...
     * End container.
     *
     * @param name The local name (i.e. without the platform ID) of the agent to
     * create.
     * @param className The fully qualified name of the class implementing the
     * agent to start.
     ...
     * @param name The local name (i.e. without the platform ID) of the agent to
     * kill.
     * @throws NotFoundException If no agent with the given local name are
     * running within the active Front End.
     */
    ...
    /**
     * Add a listener that will be notified about Front-End relevant events such
     * as BORN_AGENT and DEAD_AGENT
     * <br>
     * <b>NOT available in MIDP</b>
     * <br>
     *
     * @param l The listener that will be notified about Front-End relevant
     * events
     */
    ...
    /**
     * Disconnect this front-end container from the platform. That is, though
     * the local front-end container and its agent(s) remain active, the
     * back-end is shut down and, of course, the FE-2-BE connection is closed
     * --> Therefore the rest of the platform does not see the local container
     * and its agent(s) anymore. The local container automatically re-connects
     * to the platform (and local agents are registered with the AMS again) as
     * soon as a message to a remote agent is sent.
     */
    ...
    /**
     * Activate the Java environment terminator when the JADE runtime has
     * stopped.
     */
