 ...
 * @version $Date: 2015-03-28 00:03:37 +0100 (sab, 28 mar 2015) $ $Revision:
 * 6751 $
 *
 */
public class MainContainerImpl implements MainContainer, AgentManager {
    ...
    /**
     * Profile option that specifies the classes of agents that must be
     * restarted in case of fault of the master Main Contaier
     */
     ...
     * shutting down the platform. Considering that there can be thousands of
     * such nodes, explicitly killing them all may lead to very long shut down
     * time. Possible values: true, false (default)
     */
    ...
    public MTPDescriptor installMTP(String address, ContainerID cid, String className) throws NotFoundException, UnreachableException, MTPException {
         ...
         * dsc.getAddresses()[i] + "]"); } System.out.println("Supported
         * Protocols: "); for(int i = 0; i < dsc.getSupportedProtocols().length;
         * i++) { System.out.println("[" + dsc.getSupportedProtocols()[i] +
         * "]"); } *
         */
     ...
     * platform. If the modification implies a change in the agent state (and
     * the agent lives in the platform) --> force that change If the
     * modification implies a change in the agent ownership (and the agent lives
     * in the platform) --> force that change
     */
