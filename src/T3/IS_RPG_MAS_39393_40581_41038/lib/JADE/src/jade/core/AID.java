...
/**
 * This class represents a JADE Agent Identifier. JADE internal agent tables use
 * this class to record agent names and addresses.
 */
public class AID implements Comparable, Serializable {
    ...
    /**
     * Constructor for an Agent-identifier This constructor (which is
     * deprecated), examines the name to see if the "@" chararcter is present.
     * If so, it calls      <code> this(name, ISGUID)<code>
     * otherwise it calls <code>this(name, ISLOCALNAME)</code> This ensures
     * better compatibility with JADE2.2 code.
     *
     * @param guid is the Globally Unique identifer for the agent. The slot name
     * assumes that value in the constructed object.
     * @deprecated This constructor might generate a wrong AID, if the passed
     * parameter is not a guid (globally unique identifier), but the local name
     * of an agent (e.g. "da0").
     ...
     * @param isGUID indicates if the passed <code>name</code> is already a
     * globally unique identifier or not. Two constants <code>ISGUID</code>,
     * <code>ISLOCALNAME</code> have also been defined for setting a value for
     * this parameter. If the name is a local name, then the HAP (Home Agent
     * Platform) is concatenated to the name, separated by "@".
     *
     */
    ...
    /**
     * Key to retrieve the agent class name as a user defined slot of the AID
     * included in the AMSAgentDescription registered with the AMS.
     */
    public static final String AGENT_CLASSNAME = "JADE-agent-classname";

    /**
     * This method permits to set the symbolic name of an agent. The passed
     * parameter must be a GUID and not a local name.
     */
    ...
    /**
     * This method permits to set the symbolic name of an agent. The passed
     * parameter must be a local name.
     */
    ...
    /**
     * This method permits to add a transport address where the agent can be
     * contacted. The address is added only if not yet present
     */
     ...
     *
     * @return all the user-defined slots as a
     * <code>jade.util.leap.Properties</code> java Object.
     ...
     * <ul>
     * <li> The argument <code>o</code> is an <code>AID</code> object with the
     * same <em>GUID</em> in its name slot (apart from differences in case).
     * <li> The argument <code>o</code> is a <code>String</code> that is equal
     * to the <em>GUID</em> contained in the name slot of this Agent ID (apart
     * from differences in case).
     * </ul>
     */
    ...
    /**
     * Comparison operation. This operation imposes a total order relationship
     * over Agent IDs.
     *
     * @param o Another <code>AID</code> object, that will be compared with the
     * current <code>AID</code>.
     * @return -1, 0 or 1 according to the lexicographical order of the
     * <em>GUID</em> of the two agent IDs, apart from differences in case.
     */
     ...
     * <code>AID</code> objects with equal names or with names differing only in
     * case have the same hash code.
     *
    ...
    /**
     * Returns the local name of the agent (without the HAP). If the agent is
     * not local, then the method returns its GUID.
     */
    ...
    /**
     * Returns the HAP of the agent or null if the GUID of this <code>AID</code>
     * is not of the form <local-name>@<platform-name>
     */
