...
public class UDPNodeMonitoringService extends NodeMonitoringService {
     ...
     * hostname where the Main Container is listening for UDP pings. If not
     * specified the value of the Profile.LOCAL_HOST option is used. If this is
     * null too the default network name is used.
     */
    public static final String HOST = PREFIX + "host";

    /**
     * This constant is the name of the property whose value contains a boolean
     * indication that specifies whether or not UDP pings must be accepted on
     * the indicated local-host only (default = false).
     */
    ...
    /**
     * This constant is the name of the property whose value contains an integer
     * representing the time interval (in milliseconds) in which a peripheral
    ...
    /**
     * This constant is the name of the property whose value contains an integer
     * representing the maximum time (in milliseconds) the main container waits
     * for a ping message before considering the peripheral container
     * unreachable.<br>
    ...
    /**
     * This constant is the name of the property whose value contains an integer
     * representing the maximum time a node can stay unreachable after it gets
     * removed from the platform.<br>
     * This property is only meaningful on a main container.
     */
    ...
    /**
     * This constant is the name of the property whose value contains an integer
     * representing the number of UDP ping packets that must be received from an
     * un-monitored node before considering it an orphan node and issuing an
     * Orphan-Node Vertical command.<br>
    ...
    /**
     * This constant is the name of the property whose value contains an integer
     * representing the maximum number of UDP ping packets received from an
     * un-monitored node that are traced. Successive packets from the same
     * un-monitored node will be completely ignored The default for this
     * property is 100.<br>
     * This property is only meaningful on a main container.
     */
    ...
    /**
     * This constants is the name of the property whose value contains the fully
     * qualified class name of a concrete implementation of the
     * <code>NetworkChecker</code> interface. If this property is set, before
     * considering dead a monitored node that remains unreachable for more than
     * the <code>unreachable-limit</code>, the specified
     * <code>NetworkChecker</code> is activated to check if the lack of UDP ping
     * packets depends on a network disconnection. If this is the case the node
     * is kept in the <code>UNREACHABLE</code> state and is not removed.<br>
     * This property is only meaningful on a main container.
     *
     * @see NetworkChecker
     */
    ...
    /**
     * Vertical command issued on the Main Container when a given number of ping
     * packets are received from an unknown node
     */
    ...
    /**
     * Extracts an integer value from a given profile. If the value is less than
     * zero it returns the specified default value
     *
     * @param p profile
    ...
    /**
     * Inner class ServiceComponent A slice is needed to process the
     * H_ACTIVATEUDP and H_DEACTIVATEUDP horizontal commands that deal with UPD
     * Monitor clients activation
     */
    ...
    /**
     * Inner class UDPMonitorIncomingFilter An incoming filter is needed to
     * react to failures of main containers that are monitoring this node by
     * stopping the related UDP Monitor clients. This main container is either
     * the main container a node is attached to or a main container replica if
     * the Main Replication Service is active
     */
