...
#MIDP_INCLUDE_END*/
     ...
     * delivery of a message fails, no failure handling action must be
     * performed.
     */
    ...
    /**
     * User defined parameter key specifying, when set to "true", that if the
     * delivery of a message fails, no FAILURE notification has to be sent back
     * to the sender. This differs from IGNORE_FAILURE since it does not inhibit
     * the delivery failure handling mechanism (based on the NOTIFY_FAILURE
     * VCommand) at all, but just the delivery of the automatic AMS FAILURE
     * reply.
     */
    public static final String DONT_NOTIFY_FAILURE = "JADE-dont-notify-failure";

    /**
     * User defined parameter key specifying that the JADE tracing mechanism
     * should be activated for this message.
     */
    public static final String TRACE = "JADE-trace";

    /**
     * User defined parameter key specifying that this message does not need to
     * be cloned by the message delivery service. This should be used ONLY when
     * the message object will not be modified after being sent
     */
    ...
    /**
     * User defined parameter key specifying that this message must be stored
     * for a given timeout (in ms) in case it is sent to/from a temporarily
     * disconnected split container. After that timeout a FAILURE message will
     * be sent back to the sender.<br>
    ...
    /**
     * These constants represent the expected size of the 2 array lists used by
     * this class *
     */
    ...
    /**
     * @deprecated Since every ACL Message must have a message type, you should
     * use the new constructor which gets a message type as a parameter. To
     * avoid problems, now this constructor silently sets the message type to
     ...
     * performative. If the passed integer does not correspond to any of the
     * known performatives, it silently initializes the message to
     * <code>not-understood</code>.
     *
     */
    ...
    /**
     * Writes the <code>:sender</code> slot. <em><b>Warning:</b> no checks are
    ...
    /**
     * Removes all values from <code>:receiver</code> slot. <em><b>Warning:</b>
     * no checks are made to validate the slot value.</em>
     */
    ...
    /**
     * Removes all values from <code>:reply_to</code> slot. <em><b>Warning:</b>
     * no checks are made to validate the slot value.</em>
     */
     ...
     * Remind to use the set of constants (i.e. <code> INFORM, REQUEST, ...
     * </code>) defined in this class
     */
    ...
    /**
     * Writes the <code>:content</code> slot. <em><b>Warning:</b> no checks are
     * made to validate the slot value.</em>
     * <p>
     * <p>
     * Notice that, in general, setting a String content and getting back a byte
     * sequence content - or viceversa - does not return the same value, i.e.
     * the following relation does not hold      <code>
    ...
    //#MIDP_EXCLUDE_BEGIN
    /**
     * This method sets the content of this ACLMessage to a Java object. It is
     * not FIPA compliant so its usage is not encouraged. For example:<br>
     * <PRE>
     ...
     * </PRE>
     *
     * @param s the object that will be used to set the content of the
     * ACLMessage.
     ...
     * compliant so its usage is not encouraged. For example to read Java
    ...
    /**
     * Writes the <code>:reply-with</code> slot. <em><b>Warning:</b> no checks
    ...
    /**
     * Writes the <code>:in-reply-to</code> slot. <em><b>Warning:</b> no checks
     * are made to validate the slot value.</em>
     *
     * @param reply The new value for the slot.
    ...
    /**
     * Add a new user defined parameter to this ACLMessage. Notice that
     * according to the FIPA specifications, the keyword of a user-defined
     * parameter must not contain space inside. Note that the user does not need
     * to (and shall not) add the prefix "X-" to the keyword. This is
    ...
    /**
     * Searches for the user defined parameter with the specified key. The
     * method returns <code>null</code> if the parameter is not found.
     *
    ...
    /**
     * Return all user defined parameters of this ACLMessage in form of a
     * Properties object
     *
     */
     ...
     *
     * @param key the key that needs to be removed
     * @return the value to which the key had been mapped or null if the key was
     * not present
     */
...
//	#CUSTOM_EXCLUDE_BEGIN
     ...
     * <code>:sender</code> and <code>:receiver</code> message slots to fill in
     * the envelope.
     *
...
//	#CUSTOM_EXCLUDE_END
     ...
     * representation of this <code>ACLMessage</code> into a character string.
     * If the content is a bytesequence, then it is automatically converted into
     * Base64 encoding.
     *
    ...
    /**
     * create a new ACLMessage that is a reply to this message. In particular,
     * it sets the following parameters of the new message: receiver, language,
     * ontology, protocol, conversation-id, in-reply-to, reply-with. The
     * programmer needs to set the communicative-act and the content. Of course,
     ...
     *
     * @return An Iterator over all the intended receivers of this message
     * taking into account the Envelope ":intended-receiver" first, the Envelope
