 ...
 * agents.
 * <br> The first one is to extend the <code>GatewayAgent</code> (see its
 * javadoc for reference).
 * <br> The second one is to extend this <code>GatewayBehaviour</code> and add
 * an instance of this Behaviour to your own agent that will have to function as
 * a gateway.
 *
...
public abstract class GatewayBehaviour extends CyclicBehaviour {
     ...
     * releaseCommand) In this Hash, the key is the Object while the value is
     * the Event
     */
     ...
     * </code>
     * </p>
     * <b> REMIND THAT WHEN THE COMMAND HAS BEEN PROCESSED, YOU MUST CALL THE
     * METHOD <code>releaseCommand</code>. Sometimes, you might prefer launching
     * a new Behaviour that asynchronously processes this command and release
     * the command just when the Behaviour terminates, i.e. in its
     * <code>onEnd()</code> method.
     *
     */
    abstract protected void processCommand(Object command);

    /**
     * notify that the command has been processed and remove the command from
     * the queue
     *
