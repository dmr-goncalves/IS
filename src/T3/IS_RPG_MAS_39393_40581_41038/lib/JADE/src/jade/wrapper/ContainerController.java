 ...
 * Invoking methods on instances of this class, it is possible to request
 * services from <it>in-process</it> agent containers. This class must not be
 * instantiated by applications. Instead, use the <code>createContainer()</code>
 * method in class <code>Runtime</code>.
 * <br>
...
public class ContainerController {
    ...
    /**
     * This constructor requires a concrete implementation of a JADE agent
     * container, which cannot be instantiated by applications, so it cannot be
     * meaningfully called from application code. The proper way to create an
     * agent container from an application is to call the
     ...
     * @throws ControllerException If any problems occur obtaining this proxy or
     * if no such agent exists in the local container.
     */
    ...
    /**
     * Get a controller (i.e. a proxy) to a local agent given its local-name or
     * GUID.
     *
     * @param name The local name or the GUID of the desired agent.
     * @param isGuid A flag indicating whether <code>name</code> represents the
     * local-name (<code>AID.ISLOCALNAME</code>) or the GUID
     * (<code>AID.ISGUID</code>) of the desired agent.
     * @throws ControllerException If any problems occur obtaining this proxy or
     * if no such agent exists in the local container.
     */
     ...
     * The agent will be given a FIPA compliant agent identifier using the
     * nickname and the ID of the platform it is running on.
     * @param className The fully qualified name of the class that implements
     * the agent.
     * @param args An object array, containing initialization parameters to pass
     * to the new agent.
     * @return A proxy object, allowing to call state-transition forcing methods
     * on the real agent instance.
     */
    ...
    // HP Patch begin ----------------------------------------------------------------------------------
    /**
     * Add an Agent to this container. Typically Agent would be some class
     * extending Agent which was instantiated and configured.
     *
     * @param nickname A platform-unique nickname for the newly created agent.
     * The agent will be given a FIPA compliant agent identifier using the
     * nickname and the ID of the platform it is running on.
     * @param anAgent The agent to be added to this agent container.
     * @return An AgentController, allowing to call state-transition forcing
    ...
    /**
     * Installs a new message transport protocol, that will run within this
     * container.
     *
     * @param address The transport address exported by the new MTP, in string
     * format.
     * @param className The fully qualified name of the Java class that
     * implements the transport protocol.
     * @exception MTPException If something goes wrong during transport protocol
     * activation.
     */
     ...
     * @exception MTPException If something goes wrong during transport protocol
     * activation.
     * @exception NotFoundException If no protocol with the given address is
     * currently installed on this container.
     */
    ...
    /**
     * Retrieve a controller for the platform the container wrapped by this
     * ContainerController belongs to and acts as the Main Container.
     *
     * @return a <code>PlatfromController</code> for the platform the container
     * wrapped by this <code>ContainerController</code> belongs to.
     * @exception ControllerException If the container wrapped by this
     * <code>ContainerController</code> is not valid or is not the platform Main
     * Container.
     */
