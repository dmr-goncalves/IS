...
/**
 * When using a single connection between FE and BE it is important to ensure
 * that when FE writes JICP packet and waits for the Response, no IN Command is
 * written by the BE in the meanwhile. This could be the case in the mediator
 * startup phase as exemplified below: FE BE BackEndContainer CREATE_MEDIATOR
 * ----------------------------------> Start ------------------------> Start
 * Agent A1 |----------------> | Message for A1 messageIn()
 * <--------------------- COMMAND <------------------------|
 * <---------------------------------- | Other stuff (e.g. | start of other
 * agents) Start completed | CREATE_MEDIATOR_RESPONSE <------------------------
 * <----------------------------------
 *
 * The situation is even worse when the GET_SERVER_TIME request is sent just
 * after mediator startup. This class simply wraps a NIOJICPConnection and
 * forwards to it all calls to public methods. Furthermore it allows a Thread to
 * lock the connection so that if another Thread tries to write a packet it is
 * enqueued until the connection is unlocked.
 */
