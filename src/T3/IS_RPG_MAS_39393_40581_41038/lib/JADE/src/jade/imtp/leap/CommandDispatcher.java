 ...
 * Watzke and Giovanni Caire (TILAB), 09/11/2000.</p>
 *
...
class CommandDispatcher implements StubHelper, ICP.Listener {
     ...
     * <tt>CommandDispatcher</tt> that have not get an unique name by their
     * container yet.
     */
     ...
     * dispatcher to the skeletons for these objects. It is used when a command
     * is received from a remote JVM.
     */
    protected Map skeletons = new HashMap();

    /**
     * This hashtable maps the objects remotized by this command dispatcher to
     * their IDs. It is used when a stub of a remotized object must be built to
     * be sent to a remote JVM.
     */
    protected Map ids = new HashMap();

    /**
     * A counter that is used for determining IDs for remotized objects.
     * Everytime a new object is registered by the command dispatcher it gets
     * the value of this field as ID and the field is increased.
     */
    ...
    /**
     * The stub for the platform service manager. This stub will be shared by
     * all nodes within this Java virtual Machine.
     */
    ...
    /**
     * A sole constructor. To get a command dispatcher the constructor should
     * not be called directly but the static <tt>create</tt> and
     * <tt>getDispatcher</tt> methods should be used. Thereby the existence of a
     * singleton instance of the command dispatcher will be guaranteed.
     */
    ...
    /**
     * Dispatches the specified serialized command to one of the specified
     * transport addresses (the first where dispatching succeeds) directly or
     * through the router.
     *
     * @param destTAs a list of transport addresses where the command dispatcher
     * should try to dispatch the command.
    ...
    /**
     * Dispatches the specified serialized command to one of the specified
     * transport addresses (the first where dispatching succeeds) directly.
     *
     * @param destTAs a list of transport addresses where the command dispatcher
     * should try to dispatch the command.
     * @param commandPayload the serialized command that is to be dispatched.
    ...
    /**
     * Dispatches the specified serialized command to one of the specified
     * transport addresses (the first where dispatching succeeds) through the
     * router.
     *
     * @param destTAs a list of transport addresses where the command dispatcher
     * should try to dispatch the command.
     ...
     * supported by the ICPs currently installed in the command dispatcher. If
     * there is no ICP installed to the command dispatcher or their transport
     * protocols are not able to convert the specified URL a
     * <tt>DispatcherException</tt> is thrown.
     *
     ...
     * dispatcher or the transport protocols of the ICPs are not able to convert
     * the specified URL.
     */
     ...
     *
     * @param ta the transport addresses where the command should be sent.
    ...
    // /////////////////////////////////////////
    /**
     * Handles a received (still serialized) command object, i.e. deserialize it
