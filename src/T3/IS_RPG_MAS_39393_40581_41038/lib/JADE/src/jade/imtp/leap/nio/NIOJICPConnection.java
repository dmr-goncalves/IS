...
public class NIOJICPConnection extends Connection {
    ...
    /**
     * Read a JICPPacket from the connection. The method is synchronized since
     * we reuse the same Buffer object for reading the packet header. It should
     * be noted that the packet data may not be completely available when the
     * embedded channel is ready for a READ operation. In that case a
     * PacketIncompleteException is thrown to indicate that successive calls to
     * this method must occur in order to fully read the packet.
     */
     ...
     * }. When the buffer returned by {@link #preprocessBufferToWrite(java.nio.ByteBuffer)
    ...
    /**
     * Inner class BufferTransformerInfo This class keeps together a
     * BufferTransformer and a ByteBuffer holding data already received but not
     * yet processed by that transformer. Such data will be used (together with
     * newly received data) at next transformation attempt
     */
