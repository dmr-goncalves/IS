 ...
 * an asynchronous way using java.nio
 * <br><br>
 ...
 * <ul>
 * <li>
 * <code>serverid_local-host</code>: Specifies the local network interface for
 * the server socket managed by this server (defaults to localhost).
 * </li>
 * <li>
 ...
 * </li>
 * <li>
 * <code>serverid_protocol</code>: Specifies the protocol used by this server in
 * the form of <code>jade.imtp.leap.JICP.ProtocolManager<code>
 * class
 * </li>
 * <li>
 ...
 * </li>
 * <li>
 * <code>serverid_leap-property-file</code>: Specifies the leap-property file to
 * be used by this server.
 * </li>
 * <li>
 * <ode>serverid_poolsize</code>: Specifies the number of threads used by this
 * server to manage IO events.
 * </li>
 * <ul>
 *
...
public class BEManagementService extends BaseService {
    ...
    /**
     * this property can be used to set how many bytes will be used when nio
    ...
    /**
     * This method is called by the JADE runtime just after this service has
     * been installed. It activates the IO event servers specified in the
     * Profile and the Ticker thread.
     */
    ...
    /**
     * This is called by the JADE Runtime when this service is deinstalled. It
     * stops the Ticker thread and all active IO event servers.
     */
    ...
    /**
     * Inner class IOEventServer. This class asynchronously manages a server
     * socket and all IO Events that happen on it and on all sockets opened
     * through it. The BEManagementService is basically a collection of these
     * servers.
     */
    private class IOEventServer implements PDPContextManager, PDPContextManager.Listener, JICPMediatorManager {
         ...
         * exception). This method is executed by one of the threads in the
         * server Thread pool.
         */
        ...
        /**
         * Called by a Mediator to notify that it is no longer active. This is
         * often called within the tick() method. In this case directly removing
         * the deregistering mediator from the mediators table would cause a
         * ConcurrentModificationException --> We just add the deregistering
         * mediator to a queue of mediators to be removed. The actual remotion
         * will occur at the next tick in a synchronized way.
         */
        ...
        ///////////////////////////////////////
        /**
         * If no PDPContextManager is specified in the properties file the
         * IOEventServer itself is used and the default behaviour is to issue an
         * INCOMING_CONNECTION outgoing command
         */
    ...
    private class KeyManager {
        ...
        /**
         * Read some data from the connection associated to the managed key and
         * let the IOEventServer serve it
         */
    ...
    private class LoopManager implements Runnable {
        ...
        /**
         * JDK 1.6 in Linux has a BUG so that Selector.select() in certain cases
         * do not block causing an infinite loop. The workaround is to recreate
         * the Selector. Actually we recreate the whole LoopManager
         */
