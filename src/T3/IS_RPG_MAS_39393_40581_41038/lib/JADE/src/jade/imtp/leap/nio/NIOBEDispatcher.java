...
public class NIOBEDispatcher implements NIOMediator, BEConnectionManager, Dispatcher {
    ...
    /**
     * Retrieve the ID of this mediator. Returns null if this mediator is not
     * active
     */
    public String getID() {
    ...
    /**
     * Kill the above container. This may be called by the JICPMediatorManager
     * or when a peer termination notification is received.
     */
    public void kill() {
     ...
     * @param pkt the packet that was sent by the mediated entity when opening
     * this connection
     * @param addr the address of the mediated entity
     * @param port the local port used by the mediated entity
     * @return an indication to the JICPMediatorManager to keep the connection
     * open.
     */
    ...
    /**
     * Notify this NIOMediator that an error occurred on one of the Connections
     * it was using. This information is important since, unlike normal
     * mediators, a NIOMediator typically does not read packets from connections
     * on its own (the JICPMediatorManager does that in general).
     */
    ...
    /**
     * Passes to this mediator a JICPPacket received by the JICPMediatorManager
     * this mediator is attached to. In a NIOMediator this should never be
     * called.
     */
     ...
     * information is important since, unlike normal mediators, a NIOMediator
     * may not read packets from connections on its own (the JICPMediatorManager
    ...
    /**
     * This is periodically called by the JICPMediatorManager and is used by
     * this NIOMediator to evaluate the elapsed time without the need of a
     * dedicated thread or timer.
     */
     ...
     * happen when - a packet must be dispatched to the FE. - an incoming
    ...
    /**
     * Inner class InputManager. This class manages the delivery of commands to
     * the FrontEnd
     */
    protected class InputManager {
        ...
        /**
         * Dispatch a JICP command to the FE and get back a reply. This method
         * must NOT be executed in mutual exclusion with setConnection() and
         * resetConnection() since it performs a blocking read operation --> It
         * can't just be declared synchronized.
         */
    ...
    /**
     * Inner class OutputManager This class manages the reception of commands
     * and keep-alive packets from the FrontEnd. This class also manages the
