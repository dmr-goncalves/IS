...
public class HTTPBEDispatcher implements BEConnectionManager, Dispatcher, JICPMediator {
    ...
    /**
     * Handle an incoming connection. This is called by the MediatorManager when
     * a CREATE or CONNECT_MEDIATOR request is received. The HTTPBEDispatcher
     * reacts to this call by resetting the current situation
     */
     ...
     * command to the FrontEnd. Mutual exclusion with itself to ensure one
     * command at a time is dispatched.
     */
    ...
    ////////////////////////////////////////////////
    /**
     * Return a stub of the remote FrontEnd that is connected to the local
     * BackEnd.
     *
    ...
    /**
     * Clean up this HTTPBEDispatcher. The shutdown process can be activated in
     * the following cases: 1) The local container is requested to exit --> The
     * exit commad is forwarded to the FrontEnd 1.a) Forwarding OK. The FrontEnd
     * sends back a response with the TERMINATED_INFO set. When this response is
     * received the shutdown() method is called (see handleJICPPacket()). 1.b)
     * Forwarding failed. The BackEndContainer ignores the exception and
     * directly calls the shutdown() method.
     *
     * Note that in the case the FrontEnd spontaneously exits and in the case
     * the max disconnection time expires the kill() method is called --> see
     * case 1.
     */
    ...
    /**
     * Inner class OutgoingsHandler. This class manages outgoing commands i.e.
     * commands that must be sent to the FrontEnd.
     *
     * NOTE that, since HTTPBEDispatcher.dispatch() is synchronized only one
     * thread at a time can execute the deliverCommand() method. This also
     * ensures that only one thread at a time can execute the dispatchResponse()
     * method. As a consequence it's impossible that at a certain point in time
     * there is both a thread waiting for a command and a thread waiting for a
     * response.
     */
    private class OutgoingsHandler implements TimerListener {
         ...
         *
         * @exception ICPException if 1) the frontEndStatus is not REACHABLE, 2)
         * the response timeout expires (the frontEndStatus is set to
         * UNREACHABLE) or 3) the OutgoingsHandler is reset (the frontEndStatus
         * is set to CONNECTING). Called by HTTPBEDispatcher#dispatch()
         */
        ...
        /**
         * Dispatch a response received from the FrontEnd to the issuer of the
         * command this response refers to. If no one is waiting for this
         * response (the frontEndStatus must be different from REACHABLE), set
         * the frontEndStatus to REACHABLE. Then wait for the next command to
         * transfer to the FrontEnd.
         *
         * @return the next outgoing command to be transferred to the FrontEnd
         * or null if the OutgoingsHandler is reset. Called by
         * HTTPBEDispatcher#handleJICPPacket()
         */
         ...
         * If there is a thread waiting for a command to deliver to the FrontEnd
         * it will return null. If there is a thread waiting for a response it
         * will exit with an Exception. The frontEndStatus is set to CONNECTING.
