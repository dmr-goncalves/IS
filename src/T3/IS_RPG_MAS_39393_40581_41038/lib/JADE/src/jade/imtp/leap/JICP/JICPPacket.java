...
/**
 * This class is the JICP data packet representation along with methods for
 * reading from and writing to dataXXputStreams.
 *
...
public class JICPPacket {
     ...
     * A JICPServer receiving a JICPPacket from a remote container interprets
     * this field as the ID of a local Mediator. - A Mediator receiving a
     * JICPPacket from its mediated container interprets this field as the
     * serialized transport address of final destination to forward the packet.
     */
    ...
    /**
     * constructs a JICPPacket of type JICPProtocol.ERROR_TYPE and sets the data
    ...
    /**
     * Writes the packet into the provided <code>OutputStream</code>. The packet
     * is serialized in an internal representation, so the data should be
     * retrieved and deserialized with the <code>readFrom()</code> static method
     * below. The output stream is flushed but not opened nor closed by this
     * method.
     *
    ...
    /**
     * This static method reads from a given <code>DataInputStream</code> and
     * returns the JICPPacket that it reads. The input stream is not opened nor
