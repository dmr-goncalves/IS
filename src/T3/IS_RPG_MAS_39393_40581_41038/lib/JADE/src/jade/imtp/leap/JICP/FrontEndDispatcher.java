...
public class FrontEndDispatcher implements FEConnectionManager, Dispatcher, TimerListener, Runnable {
    ...
    /**
     * subclasses may overwrite this to provide their version of a
     * JICPConnection
     *
     * @param ta
     * @return
    ...
    /**
     * Start the asynchronous reconnection process implemented in the run()
     * method
     */
    ...
    /**
     * Refresh the connection drop-down timer. Mutual exclusion with
     * clearTimers(), updateKeepAlive() and doTimeOut()
     */
    ...
    /**
     * Send a DROP_DOWN packet to the BE and close the connection. Mutual
     * exclusion with dispatch() and sendKeepAlive().
     */
    ...
    /**
     * Inner class IncomingCommandServer Serving incoming commands
     * asynchronously is necessary to support commands whose serving process
     * involves issuing one or more outgoing commands. If such commands were
     * served directly by the ConnectionReader thread, in facts, there would be
     * no chance to get the response to triggered outgoing commands.
     */
