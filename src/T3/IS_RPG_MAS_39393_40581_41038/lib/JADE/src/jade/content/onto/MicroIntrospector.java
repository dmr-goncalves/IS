...
/**
 * This introspector does nothing but "asking objects to convert themselves
 * into/from abstract descriptors". It could be used when working in Java
 * environments where the reflection is not supported (MIDP). All classes in an
 * ontology using this introspector must implement the
 * <code>Introspectable</code> interface.
 *
...
public class MicroIntrospector implements Introspector {
     ...
     * (instances of that class) can be accomplished by this introspector. This
     * is the case if <code>javaClass</code> implements the
