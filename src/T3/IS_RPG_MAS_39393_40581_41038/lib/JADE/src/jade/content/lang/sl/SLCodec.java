...
public class SLCodec extends StringCodec {
    ...
    /**
     * Create an SLCodec for the full SL-language (FIPA-SL) specifying whether
     * or not java primitive types (long, int, float, double) must be preserved.
     * This is achieved by encoding long values as <numeric-value>L and float
     * values as <numeric-valueF. It should be noticed that this encoding is NOT
     * FIPA SL standard @param preserveJavaTypes Indicates whether or not java
     * primitive types must be
     *
     * preserved
     */
    ...
    /**
     * Create an SLCodec for the given profile of SL-language specifying whether
     * or not java primitive types (long, int, float, double) must be preserved.
     *
     * @param slType specify 0 for FIPA-SL0, 1 for FIPA-SL1, 2 for FIPA-SL2, any
     * other value can be used for full FIPA-SL
     * @param preserveJavaTypes Indicates whether or not java primitive types
     * must be preserved
     */
    ...
    /**
     * Encode a string, taking care of quoting separated words and escaping
     * strings, if necessary. And append it to the buffer.
     *
     */
    ...
    /**
     * Decodes the content to an abstract description, where the content is
     * known to be a Term.
     *
     * @param ontology the ontology.
    ...
    /**
     * @return the ontology containing the schemas of the operator defined in
     * this language
     */
    ...
    /**
     * Encode the slots of an abstract descriptor by order, i.e. without writing
     * the slot names. Also take into account that, in order to ensure a correct
     * parsing, empty slots can only occur at the end. Append this encoded
     * string to buffer.
     */
     ...
     * non-empty slot the slot name followed by the slot value. Append this
     * encoded string to buffer.
     */
     ...
     * object from the stream and is preparing to return it to the caller. <br>
     * The readResolve method is not invoked on the object until the object is
     * fully constructed.
     */
