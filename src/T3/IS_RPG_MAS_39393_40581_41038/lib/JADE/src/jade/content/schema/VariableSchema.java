...
public class VariableSchema extends TermSchema {
    ...
    /**
     * Construct a schema that vinculates an entity to be a generic variable.
     * Note that there are no different types of variable as it happens for
     * concepts (e.g. Person, Address...), IREs (e.g. IOTA, ANY, ALL...) and the
     * like. Therefore there is no VariableSchema constructor that takes a
     * String parameter. Not also that the type of the values that can be
     * assumed by the variable is another story and is defined by the
    ...
    /**
     * A variable can be put whereever a term of whatever type is required --> A
     * VariableSchema is compatible with s if s descends from
