 ...
 * received all the result notifications from the responders, the initiator may
 * send further initiation messages. The session of such a protocol with a given
 * responder terminates when one of the followings occurs: i) The initiator
 * sends an explicit CANCEL message instead of the next initiation message to
 * the responder. ii) The responder replies with a negative reply i.e. REFUSE,
 * NOT_UNDERSTOOD or FAILURE ii) The responder attaches a termination flag to an
 * INFORM result notification. That termination flag can be detected using the
...
public class IteratedAchieveREInitiator extends AchieveREInitiator {
    ...
    /**
     * Construct an <code>IteratedAchieveREInitiator</code> with an empty
     * DataStore
     */
    ...
    /**
     * Construct an <code>IteratedAchieveREInitiator</code> with a given
     * DataStore
     *
    ...
    /**
     * This method is called every time an <code>inform</code> message is
     ...
     * @param nextRequest the Vector of ACLMessage objects to be sent at next
     * round
     */
    ...
    /**
     * This method is called when all the result notification messages of the
     * current round have been collected. By result notification message we
     * intend here all the <code>inform,
     * failure</code> received messages, which are not out-of-sequence according
     * to the protocol rules. This default implementation does nothing;
     * programmers might wish to override this method in case they need to react
     * to this event by analysing all the messages in just one call.
     *
     ...
     * registered behaviour can retrieve the received <code>inform</code>
     * ACLMessage object from the datastore at the <code>REPLY_KEY</code> key
     * and the Vector of ACLMessage objects to be sent at next round at the
     * <code>ALL_NEXT_REQUESTS_KEY</code>.
     *
     ...
     * <code>ALL_RESULT_NOTIFICATIONS_KEY</code> key and the Vector of
     * ACLMessage objects to be sent at next round at the
     * <code>ALL_NEXT_REQUESTS_KEY</code>.
     *
     * @param b the Behaviour that will handle this state
     */
