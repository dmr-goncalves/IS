...
/**
 * This behaviour is a simple implementation of a message receiver. It puts into
 * the given key of the given datastore the received message according to the
 * given message template and timeout. All these data must be passed in the
 * constructor. If the timeout expires before any message arrives, the behaviour
...
public class MsgReceiver extends SimpleBehaviour {
     ...
     * @param msgKey the key where the beahviour must put the received message
     * into the DataStore.
     *
     */
     ...
     * <code>INTERRUPTED</code> if this <code>MsgReceiver</code> was interrupted
     * calling the <code>interrupt()</code> method.
     *
     */
    public int onEnd() {
        received = false;
     ...
     * received or the timeout has expired (the <code>msg</code> parameter is
     * null in this case). Users may redefine this method to react to this
     * event. The default implementation of does nothing.
     */
    protected void handleMessage(ACLMessage msg) {
    }

    /**
     * Reset this behaviour, possibly replacing the receive templatt and other
     * data.
     *
     * @param mt The template to match ACL messages against during the receive
     * operation.
     * @param deadline The relative timeout of the receive operation. If the
     * <code>INFINITE</code> constant is used, then no deadline is set and the
     * operation will wait until a matching ACL message arrives.
     * @param s The datastore where the received ACL message is to be put.
     * @param msgKey The key to use to put the received message into the
     * selected datastore.
     */
    ...
    /**
     * This method allows modifying the key in the DS where to put the received
     * message
     *
     */
