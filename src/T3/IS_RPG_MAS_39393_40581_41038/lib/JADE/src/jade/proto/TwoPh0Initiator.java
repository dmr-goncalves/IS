...
public class TwoPh0Initiator extends Initiator {
    ...
    /**
     * key to retrieve from the DataStore of the behaviour the vector of all
     * messages that have been received as response.
     */
    ...
    /**
     * key to retrieve from the DataStore of the behaviour the vector of CFP
     * messages for which a response has not been received yet.
     */
     ...
     * @param cfp The message that must be used to initiate the protocol. Notice
     ...
     * @param outputKey Data store key where the behaviour will store the Vector
     * of messages to be sent to initiate the successive phase.
     * @param store <code>DataStore</code> that will be used by this
     * <code>TwoPh0Initiator</code>.
     */
     ...
     * return a vector of CFP objects for 1:N conversations.
     *
     * @param cfp the ACLMessage object passed in the constructor
     * @return a vector of ACLMessage objects. The value of the
     * <code>reply-with</code> slot is ignored and regenerated automatically by
     * this class. Instead user can specify <code>reply-by</code> slot
     * representing phase0 timeout.
     */
     ...
     * intend here all the <code>propose, failure, not-understood</code>
     * received messages, which are not out-of-sequence according to the
     * protocol rules. This default implementation does nothing; programmers
     * might wish to override this method to modify the Vector of initiation
     * messages (<code>nextPhMsgs</code>) for next phase. More in details this
     * Vector already includes messages with the performative set according to
     * the default protocol rules i.e. QUERY_IF (if all responders replied with
     * PROPOSE) or REJECT_PROPOSAL (if at least one responder failed or didn't
     * reply). In particular, by setting the <code>reply-by</code> slot, users
     * can specify a timeout for next phase.
     *
     ...
     * been received yet
     * @param nextPhMsgs The Vector of initiation messages for next phase
     * already filled with <code>QUERY_IF</code> messages (if all responders
     * replied with <code>PROPOSE</code>) or <code>REJECT_PROPOSAL</code> (if at
     * least one responder failed or didn't reply).
     */
     ...
     * responsibility of the registered behaviour to put the <code>Vector</code>
     * of ACLMessage objects to be sent into the datastore at the
     ...
     * <code>Behaviour</code> to the DataStore of this current behaviour. The
     * registered behaviour can retrieve the vector of ACLMessage proposes,
     * failures, pending and responses from the datastore at
     * <code>ALL_PROPOSES_KEY</code>, <code>ALL_FAILURES_KEY</code>,
    ...
    /**
     * This method sets for all prepared cfps <code>conversation-id</code> slot
     * (with value passed in the constructor), <code>protocol</code> slot and
     * <code>reply-with</code> slot with a unique value constructed by
     * concatenating receiver's agent name and phase number (i.e. 0). After that
     * it sends all cfps.
     *
    ...
    /**
     * Check whether a reply is in-sequence and than update the appropriate
     * Session and removes corresponding cfp from vector of pendings.
     *
    ...
    class Session implements ProtocolSession, Serializable {
         ...
         *
         * @param perf
         * @return Return true if received ACLMessage is consistent with the
         * protocol
         */
