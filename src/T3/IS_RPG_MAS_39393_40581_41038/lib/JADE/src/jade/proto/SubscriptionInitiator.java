...
/**
 * This is a single homogeneous and effective implementation of the initiator
 ...
 * <p>
 * The structure of these protocols is always the same. The initiator sends a
 * "subscription" message (in general it performs a communicative act).
 * <p>
 * The responder can then reply by sending a <code>not-understood</code>, a
 * <code>refuse</code> or an <code>agree</code> message to communicate that the
 * subscription has been agreed. This first category of reply messages has been
 * here identified as a "response". Sending no response is allowed and is
 * equivalent to sending an <code>agree</code>.
 * <p>
 * Each time the condition indicated within the subscription message becomes
 * true, the responder sends proper "notification" messages to the initiator.
 * <p>
 * This behaviour terminates if a) neither a response nor a notification has
 * been received before the timeout set by the <code>reply-by</code> of the
 * subscription message has expired or b) all responders replied with REFUSE or
 * NOT_UNDERSTOOD. Otherwise the behaviour will run forever.
 * <p>
 ...
 * that describes the usage of this class.
 * <br> One message for every receiver is sent instead of a single message for
 * all the receivers. </i>
 *
...
public class SubscriptionInitiator extends Initiator {
    ...
    /**
     * key to retrieve from the DataStore of the behaviour the subscription
     ...
     * that the default implementation of the <code>prepareSubscription()</code>
     * method returns an array composed of only this message. The values of the
    ...
    /**
     * This method is called internally by the framework and is not intended to
     ...
     * the expiration of the timeout. This method is called internally by the
     * framework and is not intended to be called by the user
     */
    ...
    /**
     * This method must return the vector of subscription ACLMessage objects to
     * be sent. It is called in the first state of this protocol. This default
     * implementation just returns the ACLMessage object passed in the
     * constructor. Programmers might prefer to override this method in order to
     * return a vector of objects for 1:N conversations or also to prepare the
     * messages during the execution of the behaviour.
     *
     ...
     * refuse, failure</code> received messages, which are not out-of-sequence
     ...
     * the PREPARE_SUBSCRIPTIONS state. This behaviour would override the
     * homonymous method. This method also sets the data store of the registered
     ...
     * <code>ALL_SUBSCRIPTIONS_KEY</code> key. The values of the slot
     * <code>reply-with</code> is ignored and a different value is assigned
     * automatically by this class for each receiver.
     *
    ...
    /**
     * This method allows to register a user defined <code>Behaviour</code> in
     * the HANDLE_AGREE state. This behaviour would override the homonymous
     * method. This method also sets the data store of the registered
     * <code>Behaviour</code> to the DataStore of this current behaviour. The
     * registered behaviour can retrieve the <code>agree</code> ACLMessage
     ...
     * <code>ALL_RESPONSES_KEY</code> key.
     *
     * @param b the Behaviour that will handle this state
     */
    ...
    /**
     * Cancel the subscription to agent <code>receiver</code>. This method
     * retrieves the subscription message sent to <code>receiver</code> and
     * sends a suitable CANCEL message with the conversationID and all other
     * protocol fields appropriately set. The <code>:content</code> slot of this
     * CANCEL message is filled in by means of the
     * <code>fillCancelContent()</code> method. The way the CANCEL content is
     * set in fact is application specific.
     *
     * @param receiver The agent to whom we are cancelling the subscription.
     * @param ignoreResponse When receiving a CANCEL, the responder may send
     * back a response to notify that the subscription has been cancelled
     * (INFORM) or not (FAILURE). If this parameter is set to <code>true</code>
     * this response is ignored and the session with agent <code>receiver</code>
     * is immediately terminated. When <code>ignoreResponse</code> is set to
     * <code>false</code>, on the other hand, the session with agent
     * <code>receiver</code> remains active and the INFORM or FAILURE massage
     * (if any) will be handled by the <code>HANDLE_INFORM</code> and
     * <code>HANDLE_FAILURE</code> states as if they were normal notifications.
     * It is responsibility of the programmer to distinguish them and actually
     * terminate the session with agent <code>receiver</code> by calling the
    ...
    /**
     * This method is used to fill the <code>:content</code> slot of the CANCEL
     * message that is being sent to an agent to cancel the subscription
     * previously activated by means of the <code>subscription</code> message.
     * Note that all other relevant fields of the <code>cancel</code> message
     * have already been set appropriately and the programmer should not modify
     * them. The default implementation just sets a null content (the responder
     * should be able to identify the subscription that has to be cancelled on
     * the basis of the sender and conversationID fields of the CANCEL message).
     * Programmers may override this method to create an appropriate content as
    ...
    /**
     * This method should be called when the notification of a successful
     * subscription cancellation is received from agent <code>receiver</code> to
     * terminate the session with him. This method has some effect only if a
     * cancellation for agent <code>receiver</code> was previously activated by
    ...
    //#APIDOC_EXCLUDE_BEGIN
    /**
     * Initialize the data store. This method is called internally by the
     * framework and is not intended to be called by the user
     *
     */
    ...
    private static class Session implements ProtocolSession, Serializable {
        ...
        /**
         * return true if the received performative is valid with respect to the
