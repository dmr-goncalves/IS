...
/**
 * Common base class for all classes implementing the Responder role in
 * interaction protocols where the responder is expected to receive more than
 * one message from the initiator and reply to them.
 *
...
abstract class Responder extends FSMBehaviour {
     ...
     * @param mt is the MessageTemplate that must be used to match the
     * initiation message. Take care that if mt is null every message is
     ...
     * update the status of the protocol after a reply has been sent. This
    ...
    /**
     * Utility method to register a behaviour in a state of the protocol and set
     * the DataStore appropriately
     */
