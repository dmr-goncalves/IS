...
/**
 * This class implements the initiator role in a Fipa-Contract-Net or
 * Iterated-Fipa-Contract-Net interaction protocol.<br>
 * This implementation works both for 1:1 and 1:N conversation.
 * <p>
 * The following is a brief description of the protocol. The programmer should
 ...
 * <code>CFP</code> message that specifies the action to be performed and, if
 * needed, conditions upon its execution. The implementation of the callback
 * method <code>prepareCfps</code> must return the Vector of messages to be sent
 * (eventually a single message with multiple receivers).
 * <p>
 ...
 * the price or the time. Alternatively, responders may send a
 * <code>REFUSE</code>, to refuse the proposal or, eventually, a
 * <code>NOT-UNDERSTOOD</code> to communicate communication problems. This first
 * category of reply messages has been here identified as a responses and can be
 * handled via the <code>handleAllResponses()</code> callback method. Specific
 * handle callback methods for each type of communicative act are also available
 * when the programmer wishes to handle them separately:
 * <code>handlePropose(), handleRefuse(), handleNotUnderstood()</code>.
 * <p>
 * The initiator can evaluate all the received proposals and make its choice of
 * which agent proposals will be accepted and which will be rejected. This class
 * provides two ways for this evaluation. It can be done progressively each time
 * a new <code>PROPOSE</code> message is received and a new call to the
 * <code>handlePropose()</code> callback method is executed or, in alternative,
 * it can be done just once when all the <code>PROPOSE</code> messages have been
 * collected (or the <code>reply-by</code> deadline has expired) and a single
 * call to the <code>handleAllResponses()</code> callback method is executed. In
 * both cases, the second parameter of the method, i.e. the Vector
 * <code>acceptances</code>, must be filled with the appropriate
 * <code>ACCEPT/REJECT-PROPOSAL</code> messages. Notice that, for the first
 * case, the method <code>skipNextResponses()</code> has been provided that, if
 * called by the programmer when waiting for <code>PROPOSE</code> messages,
 * allows to skip to the next state and ignore all the responses and proposals
 * that have not yet been received.
 * <p>
 * Once the responders whose proposal has been accepted (i.e. those that have
 * received a <code>ACCEPT-PROPOSAL</code> message) have completed the task,
 * they can, finally, respond with an <code>INFORM</code> of the result of the
 * action (eventually just that the action has been done) or with a
 * <code>FAILURE</code> if anything went wrong. This second category of reply
 * messages has been here identified as a result notifications and can be
 * handled via the <code>handleAllResultNotifications()</code> callback method.
 * Again, specific handle callback methods for each type of communicative act
 * are also available when the programmer wishes to handle them separately:
 ...
 * handle methods or, in alternative, appropriate behaviours can be registered
 * for each handle via the <code>registerHandle</code>-type of methods. This
 * last case is more difficult to use and proper care must be taken to properly
 * use the <code>DataStore</code> of the <code>Behaviour</code> as a shared
 * memory mechanism with the registered behaviour.
 * <p>
 * When needed this class can also be used to play the initiator role in an
 * Iterated-Fipa-Contract-Net protocol. To activate a new CFP-PROPOSE iteration
 * it is sufficient to invoke the <code>newIteration()</code> method from within
 * the <code>handleAllResponses()</code> method.
 *
 * @author Giovanni Caire - TILab
 * @author Fabio Bellifemine - TILab
 * @author Tiziana Trucco - TILab
 * @author Marco Monticone - TILab
 * @version $Date: 2010-05-13 09:13:09 +0200 (gio, 13 mag 2010) $ $Revision:
 * 6337 $
 * @since JADE2.5
...
public class ContractNetInitiator extends Initiator {
    ...
    /**
     * key to retrieve from the DataStore of the behaviour the vector of CFP
     * ACLMessage objects that have to be sent.
     *
     */
     ...
     * that the default implementation of the <code>prepareCfps</code> method
    ...
    /**
     * Check the status of the sessions after the reception of the last reply or
    ...
    /**
     * This method must return the vector of ACLMessage objects to be sent. It
     * is called in the first state of this protocol. This default
     * implementation just returns the ACLMessage object (a CFP) passed in the
     * constructor. Programmers might prefer to override this method in order to
     * return a vector of CFP objects for 1:N conversations or also to prepare
     ...
     * <code>reply-with</code> is ignored and regenerated automatically by this
     * class.
     *
     */
     ...
     * This list can be filled step by step redefining this method, or it can be
     * filled at once redefining the handleAllResponses method.
     *
     */
     ...
     * collected. By result notification message we intend here all the <code>inform,
    ...
    /**
     * This method allows to register a user-defined <code>Behaviour</code> in
     * the PREPARE_CFPS state. This behaviour would override the homonymous
     * method. This method also set the data store of the registered
     * <code>Behaviour</code> to the DataStore of this current behaviour. It is
     * responsibility of the registered behaviour to put the Vector of
     * ACLMessage objects to be sent into the datastore at the
     * <code>ALL_CFPS_KEY</code> key.
     *
    ...
    /**
     * This method allows to register a user defined <code>Behaviour</code> in
     * the HANDLE_PROPOSE state. This behaviour would override the homonymous
     * method. This method also set the data store of the registered
     * <code>Behaviour</code> to the DataStore of this current behaviour. The
     * registered behaviour can retrieve the <code>propose</code> ACLMessage
     * object received from the datastore at the <code>REPLY_KEY</code> key and
     * the <code>Vector</code> of ACCEPT/REJECT_PROPOSAL to be sent back at the
     * <code>ALL_ACCEPTANCES_KEY</code>
     *
     * @param b the Behaviour that will handle this state
     */
    ...
    /**
     * This method allows to register a user defined <code>Behaviour</code> in
     * the HANDLE_REFUSE state. This behaviour would override the homonymous
     * method. This method also set the data store of the registered
     * <code>Behaviour</code> to the DataStore of this current behaviour. The
     * registered behaviour can retrieve the <code>refuse</code> ACLMessage
     * object received from the datastore at the <code>REPLY_KEY</code> key.
     *
     * @param b the Behaviour that will handle this state
     */
    ...
    /**
     * This method allows to register a user defined <code>Behaviour</code> in
     * the HANDLE_INFORM state. This behaviour would override the homonymous
     * method. This method also set the data store of the registered
     * <code>Behaviour</code> to the DataStore of this current behaviour. The
     * registered behaviour can retrieve the <code>inform</code> ACLMessage
     * object received from the datastore at the <code>REPLY_KEY</code> key.
     *
     * @param b the Behaviour that will handle this state
     */
    ...
    /**
     * This method allows to register a user defined <code>Behaviour</code> in
     * the HANDLE_ALL_RESPONSES state. This behaviour would override the
     * homonymous method. This method also set the data store of the registered
     * <code>Behaviour</code> to the DataStore of this current behaviour. The
     * registered behaviour can retrieve the vector of ACLMessage objects,
     * received as a response, from the datastore at the
     * <code>ALL_RESPONSES_KEY</code> key and the <code>Vector</code> of
     * ACCEPT/REJECT_PROPOSAL to be sent back at the
     * <code>ALL_ACCEPTANCES_KEY</code>
     *
     * @param b the Behaviour that will handle this state
     */
    ...
    /**
     * This method allows to register a user defined <code>Behaviour</code> in
     * the HANDLE_ALL_RESULT_NOTIFICATIONS state. This behaviour would override
     * the homonymous method. This method also set the data store of the
     * registered <code>Behaviour</code> to the DataStore of this current
     * behaviour. The registered behaviour can retrieve the Vector of ACLMessage
     * objects, received as a result notification, from the datastore at the
    ...
    /**
     * This method can be called (typically within the handleAllResponses()
     * method) to activate a new iteration (this means we are implementing an
     * Iterated-Contract-Net protocol).
     *
     * @param nextMessages The messages to be sent to responders at next
     * iteration. Such messages can be CFPs (for responders actually involved in
     * the next iteration) or REJECT_PROPOSALs (for responders no longer
     * involved in the next iteration).
