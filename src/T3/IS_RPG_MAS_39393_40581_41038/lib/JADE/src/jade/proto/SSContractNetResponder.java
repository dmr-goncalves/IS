...
public class SSContractNetResponder extends SSResponder {
    ...
    /**
     * Key to retrieve from the DataStore of the behaviour the last received CFP
     * ACLMessage
     */
    ...
    /**
     * Construct a SSContractNetResponder that is activated by the reception of
     * a given initiation CFP message.
     */
    ...
    /**
     * Construct a SSContractNetResponder that is activated by the reception of
     * a given initiation CFP message and uses a given DataStore.
     */
    ...
    /**
     * This method is called to handle the initial CFP message. This default
     * implementation does nothing and returns null. Programmers have to
     * override it to react to this event.
     *
     * @param cfp the initial CFP message to handle.
     * @return the reply message to be sent back to the initiator. Returning a
     * message different than PROPOSE (or returning null) terminates the
     * protocol.
     * @exception RefuseException if the CFP is refused. Throwing a
     * RefuseException has the same effect as returning a REFUSE message, but
     * automatically manages the <code>:content</code> slot.
     ...
     * Throwing a NotUnderstoodException has the same effect as returning a
     * NOT_UNDERSTOOD message, but automatically manages the
    ...
    /**
     * This method is called when an ACCEPT_PROPOSAL message is received from
     * the initiator. This default implementation does nothing and returns null.
     * Programmers have to override it to react to this event.
     *
     * @param cfp the initial CFP message.
     * @param propose the PROPOSE message sent back as reply to the initial CFP
     * message.
     * @param accept the received ACCEPT_PROPOSAL message.
     * @return the reply message to be sent back to the initiator.
     * @exception FailureException if there is an error serving the
     * ACCEPT_PROPOSAL. Throwing a FailureException has the same effect as
     * returning a FAILURE message, but automatically manages the
     * <code>:content</code> slot.
     */
     ...
     * initiator. This default implementation does nothing. Programmers may
     * override it to react to this event.
     *
     * @param cfp the initial CFP message.
     * @param propose the PROPOSE message sent back as reply to the initial CFP
    ...
    /**
     * This method is called whenever a message is received that does not comply
     * to the protocol rules. This default implementation does nothing.
     ...
     * the HANDLE_CFP state. This behaviour would override the homonymous
     ...
     * <br>
     * It is responsibility of the registered behaviour to put the reply to be
     * sent back to the initiator into the datastore at the
     * <code>REPLY_KEY</code> key. Putting a message defferent from PROPOSE (or
     * putting no message) terminates the protocol.
     *
     ...
     * <code>Behaviour</code> to the DataStore of this current behaviour.
     * <br>
     * The registered behaviour can retrieve the received
     * <code>ACCEPT_PROPOSAL</code> message from the datastore at the
     * <code>ACCEPT_PROPOSAL_KEY</code> key, the initial <code>CFP</code>
     * message at the <code>CFP_KEY</code> and the previously sent
     * <code>PROPOSE</code> message at the <code>PROPOSE_KEY</code>.
     * <br>
     * It is responsibility of the registered behaviour to put the reply to be
     * sent back to the initiator into the datastore at the
     ...
     * <br>
     * The registered behaviour can retrieve the received
     * <code>REJECT_PROPOSAL</code> message from the datastore at the
     * <code>REJECT_PROPOSAL_KEY</code> key, the initial <code>CFP</code>
     * message at the <code>CFP_KEY</code> and the previously sent
     * <code>PROPOSE</code> message at the <code>PROPOSE_KEY</code>.
     *
     * @param b the Behaviour that will handle this state
