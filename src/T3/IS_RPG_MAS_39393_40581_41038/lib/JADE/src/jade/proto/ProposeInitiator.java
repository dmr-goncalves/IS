...
/**
 * This class implements the Fipa-Propose interaction protocol with an API
 * similar and homogeneous to <code>AchieveREInitiator</code>.
 * <br>
 ...
 * indicating that it will perform some action if the Participant agrees. The
 * implementation of the callback method <code>prepareInitiations</code> must
 * return the vector of messages to be sent (eventually a single message with
 * multiple receivers).
 * <p>
 ...
 * communicate communication problems. This category of reply messages has been
 * here identified as a response and can be handled via the
 * <code>handleAllResponses</code> callback method. Specific handle callback
 * methods for each type of communicative act are also available when the
 * programmer wishes to handle them separately:
...
public class ProposeInitiator extends FSMBehaviour {
    ...
    /**
     * key to retrieve from the DataStore of the behaviour the vector of PROPOSE
     ...
     * <code>prepareInitiations()</code> method returns an array composed of
     * only this message. The values of the slot <code>reply-with</code> is
     * ignored and a different value is assigned automatically by this class for
     * each receiver.
     ...
     * implementation just returns the ACLMessage object (a PROPOSE) passed in
     * the constructor. Programmers might prefer to override this method in
     * order to return a vector of PROPOSE objects for 1:N conversations or also
     * to prepare the messages during the execution of the behaviour.
     *
    ...
    /**
     * This method is called every time an <code>accept-proposal</code> message
     * is received, which is not out-of-sequence according to the protocol
     * rules. This default implementation does nothing; programmers might wish
    ...
    /**
     * This method is called every time an <code>reject-proposal</code> message
     * is received, which is not out-of-sequence according to the protocol
     * rules. This default implementation does nothing; programmers might wish
     * to override the method in case they need to react to this event.
     *
    ...
    /**
     * This method allows to register a user defined <code>Behaviour</code> in
     * the HANDLE_ACCEPT_PROPOSAL state. This behaviour would override the
     * homonymous method. This method also set the data store of the registered
     * <code>Behaviour</code> to the DataStore of this current behaviour. The
    ...
    /**
     * This method allows to register a user defined <code>Behaviour</code> in
     * the HANDLE_REJECT_PROPOSAL state. This behaviour would override the
     * homonymous method. This method also set the data store of the registered
     * <code>Behaviour</code> to the DataStore of this current behaviour. The
     * registered behaviour can retrieve the <code>inform</code> ACLMessage
     * object received from the datastore at the <code>REPLY_KEY</code> key.
    ...
    /**
     * This method allows to register a user defined <code>Behaviour</code> in
     * the HANDLE_NOT_UNDERSTOOD state. This behaviour would override the
     * homonymous method. This method also set the data store of the registered
     * <code>Behaviour</code> to the DataStore of this current behaviour. The
     * registered behaviour can retrieve the <code>not-understood</code>
    ...
    /**
     * This method allows to register a user defined <code>Behaviour</code> in
     * the HANDLE_OUT_OF_SEQ state. This behaviour would override the homonymous
     * method. This method also set the data store of the registered
     * <code>Behaviour</code> to the DataStore of this current behaviour. The
     * registered behaviour can retrieve the <code>out of sequence</code>
     * ACLMessage object received from the datastore at the
     * <code>REPLY_KEY</code> key.
     *
     * @param b the Behaviour that will handle this state
     ...
     * intend here all the <code>accept-proposal,
     * reject-proposal, not-understood</code> received messages, which are not
     * not out-of-sequence according to the protocol rules. This default
     * implementation does nothing; programmers might wish to override the
     * method in case they need to react to this event by analysing all the
     * messages in just one call.
     *
    ...
    /**
     * Override the onStart() method to initialize the vectors that will keep
     * all the replies in the data store.
     */
    public void onStart() {
    ...
    /**
     * Override the setDataStore() method to initialize propagate this setting
     * to all children.
     */
     ...
     *
     * @param msgs A vector of ACL messages. If the first one has a non-empty
     * <code>:conversation-id</code> slot, its value is used, else a new
     * conversation identifier is generated.
     */
