 ...
 * <code>fipa-contract-net</code> interaction protocol from the point of view of
 * a responder to a call for proposal (<code>cfp</code>) message.<p>
 ...
 * <p>
 * When a message arrives that matches the message template passed to the
 * constructor, the callback method <code>prepareResponse</code> is executed
 * that must return the wished response, for instance the <code>PROPOSE</code>
 * reply message. Any other type of returned communicative act is sent and then
 * closes the protocol.
 * <p>
 * Then, if the initiator accepted the proposal, i.e. if an
 * <code>ACCEPT-PROPOSAL</code> message was received, the callback method
 * <code>prepareResultNotification</code> would be executed that must return the
 * message with the result notification, i.e. <code>INFORM</code> or
 * <code>FAILURE</code>.
 * <br>
 * In alternative, if the initiator rejected the proposal, i.e. if an
 * <code>REJECT-PROPOSAL</code> message was received, the callback method
 * <code>handleRejectProposal</code> would be executed and the protocol
 * terminated.
 * <p>
 ...
 * @version $Date: 2006-05-25 15:29:42 +0200 (gio, 25 mag 2006) $ $Revision:
 * 5884 $
 */
public class ContractNetResponder extends SSContractNetResponder {
     ...
     * <code>REJECT_PROPOSAL_KEY</code> according to the message that has been
     * received
     */
    ...
    /**
     * Redefine this method so that the HANDLE_CFP state is not registered as
     * first state
     */
