...
/**
 * Boots <B><em>JADE</em></b> system, parsing command line arguments in the old
 * (3.x) style.
 *
 * @author Giovanni Rimassa - Universita' di Parma
 * @author Dick Cowan - HP Labs
 * @version $Date: 2011-06-28 08:34:54 +0200 (mar, 28 giu 2011) $ $Revision:
 * 6419 $
 */
public class Boot3 {
     ...
     *
     * @param args The command line arguments. These use the form key:value or
     ...
     * </pre> If the arguments contain either import:x or agents:x we will
     * assume that the arguments are already in the new format and leave them
     * alone. For "import:" we test if what follows is a file name and in the
     * event it isn't we assume that it was if there are any other "-x" options
     * following.
     * <p>
     * You can't mix the old form with the new as this would make the
     * distinction between foo:bar as meaning a property named foo with a value
     * bar or an agent named foo implmented by class bar impossible.
     * <p>
    ...
    /**
     * This method verifies the configuration properties and eventually correct
     * them. It checks if the port number is a number greater than 0 otherwise
     * it throws a BootException, and if the -nomtp has been set, then delete
     * some other mtp wrongly set. If the user wants to start a platform the
     * host must be the local host so if a different name is speficied it will
     * be corrected and an exception will be thrown.
     *
