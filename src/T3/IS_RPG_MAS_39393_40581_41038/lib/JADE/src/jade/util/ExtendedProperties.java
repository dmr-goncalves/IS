 ...
 * 1) Allows specifying property values containing strings of the form
 * <b><tt>${key}</tt></b>. Such strings are properly replaced with the value of
 * the <tt>key</tt> variable that may represent another property, a system
 * property or an environment variable.
 * <p>
 * 2) Allows specifying boolean properties in the form <tt>-key</tt>. Such
 * format is equivalent to <tt>key=true</tt>.
 * <p>
 * 3) Allows importing properties from external property files by means of the
 * special key
 * <tt>import</tt>. E.g. specifying the property<br>
 * <tt>import = a-property-file-name</tt><br>
 * results in automatically adding all properties defined in the specified
 * property file
 * <p>
 * 4) Allows declaring a property as read-only. In order to do that it is
 * sufficient to end its key with a '!'. For example:
 * <pre>
 *      agentClass!=com.hp.agent.Foo
 * </pre> The value of the property can be accessed including the '!' in the key
...
public class ExtendedProperties extends Properties {
    ...
    /**
     * For testing. Simply pass command line arguments to constructor then
     * display all key=value pairs using sorted enumeration.
     */
    ...
    /**
     * Construct a ExtendedProperties object from an array of stringified
    ...
    /**
     * Add properties from a specified InputStream. Properties will be added to
    ...
    /**
     * Writes this property collection to the output stream in a format suitable
    ...
    /**
     * Add to this Properties object the stringified properties included in a
     * given array.
     *
     * @param propsStr The array of stringified properties. If null, this method
     * does nothing.
     */
    ...
    /**
     * Retrieve the position of the first valid key-value separator character
     * ('=' or ':') in a stringified property.
     *
    ...
    /**
     * Create a new Properties object by coping those properties whose key
     * begins with a particular prefix string. The prefix is removed from the
     * keys inserted into the extracted Properties object
     *
    ...
    /**
     * Get the object associated with a key. Note that, unlike getProperty(),
     * this method does not perform substitutions of variables of the form ${x}
     * in property values since such values may be non-string objects.
     *
     ...
     *
     * @param aKey The key used to store the data. The key may contain strings
     * of the form <b><tt>${key}</tt></b> which will be evaluated first.
    ...
    /**
     * Override getProperty in base class so all occurances of the form
     * <b><tt>${key}</tt></b> are replaced by their associated value.
     *
     * @param aKey Key for desired property.
    ...
    /**
     * Set property value. If value is null the property (key and value) will be
     * removed.
     *
     * @param aKey The key used to store the data. The key may contain strings
     * of the form <b><tt>${key}</tt></b> which will be evaluated first.
     * @param aValue The value to be stored, if null they property will be
     * removed.
     * @return The previous value of the specified key, or null if it did not
     * have one.
     */
    ...
    /**
     * Fetch property value for key which may contain strings of the form
     * <b><tt>${key}</tt></b>.
     *
     * @param aKey Key for desired property.
     ...
     * value string. By default, this method provides special processing on the
     * value associated with any property whose key name has the string "path"
     * as part of it (ex: "classpath", "sourcepath", "mypath"). When the value
     * for such keys is fetched any occurance of '|' will be converted to a ':'
     * on Unix systems and a ';' on Windows systems. Therefore to increase the
     * direct reuse of your property files, always use a '|' as a separator and
     * always assign a key name which has "path" as part of it.
     *
    ...
    /**
     * Extract a string value and convert it to an integer. If there isn't one
     * or there is a problem with the conversion, return the default value.
     *
     ...
     * If there isn't one or there is a problem with the conversion, return the
     * default value.
     *
     * @param aKey The key which will be used to fetch the attribute.
     ...
     * @return Non null is former value of object associated with new key. Null
     * indicates that either the existing key didn't exist or there was no
     * former value associated with the new key. i.e. null => success.
     */
    ...
    /**
     * Replace all substrings of the form ${xxx} with the property value using
     * the key xxx. Calls doSubstitutions(anInputString, false).
     *
     * @param anInputString The input string - may be null.
     * @return The resultant line with all substitutions done or null if input
     * string was.
     */
    ...
    /**
     * Replace all substrings of the form ${xxx} with the property value using
     * the key xxx. If the key is all caps then the property is considered to be
     * a system property.
     *
     * @param anInputString The input string - may be null.
     * @param allowUndefined If true, undefined strings will remain as is, if
    ...
    /**
     * Add properties from Reader. Explicitly handled so as to enable handling
     * of import=<file> directive. Blank lines as well as those beginning with a
     * '#' character (comments) are ignored.
     *
     * @param reader The buffered reader to read from. to catch circular
     * imports.
     * @throws IOException if anything goes wrong.
     */
    ...
    /**
     * Get a logical line. Any physical line ending in '\' is considered to
     * continue on the next line.
     *
     * @param reader The input reader to read.
     * @return The resultant logical line which may have been constructed from
    ...
    /**
     * Read one line from the Reader. A line may be terminated by a single CR or
     * LF, or the pair CR LF.
     *
    ...
    /**
     * List properties to provided PrintStream. Output will be in sorted key
     * sequence. If a value is null, it will appear as "key=".
     *
    ...
    /**
     * Create a String[] for the properties with one key=value pair per array
     * entry. If a value is null, it will appear as "key=".
     *
