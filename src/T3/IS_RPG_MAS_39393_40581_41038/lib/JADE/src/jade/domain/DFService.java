...
/**
 * This class provides a set of static methods to communicate with a DF Service
 * that complies with FIPA specifications. It includes methods to register,
 * deregister, modify and search with a DF. Each of this method has version with
 * all the needed parameters, or with a subset of them where, those parameters
 ...
 * <p>
 * Notice that all these methods block every activity of the agent until the
 * action (i.e. register/deregister/modify/search) has been successfully
 * executed or a jade.domain.FIPAException exception has been thrown (e.g.
 * because a FAILURE message has been received from the DF).
 * <p>
 ...
 * decodeDone(), decodeResult() and decodeNotification()</code> methods that
 * facilitate the preparation and decoding of messages to be sent/received
 * to/from the DF. The following piece of code exemplifies that in the case of
 * an agent subscribing to the default DF.
 *
...
public class DFService extends FIPAService {
     ...
     * slots, i.e. the agent name and, for each servicedescription, the service
     * name and the service type
     *
     ...
     * <p>
     * It should be noted that, depending on the policy adopted by the DF, the
     * granted lease time for the registration can be shorter than the requested
     * one. This can be checked by looking at the DF-Description actually
     * registered by the DF that is made available as the return value of this
     * method. The <code>keepRegistered()</code> method can be used to keep the
     * registration valid until a given time.
     *
     ...
     * @param dfd A <code>DFAgentDescription</code> object containing all data
     * necessary to the registration. If the Agent name is empty, than it is set
     * according to the <code>a</code> parameter.
     * @return the <code>DFAgentDescription</code> actually registered by the DF
     * @exception FIPAException If a <code>REFUSE</code>, <code>FAILURE</code>
     * or <code>NOT_UNDERSTOOD</code> message is received from the DF (to
     * indicate some error condition) or if the supplied DF-Description is not
     * valid.
     ...
     * @param dfd A <code>DFAgentDescription</code> object containing all data
     * necessary to the deregistration.
     * @exception FIPAException If a <code>REFUSE</code>, <code>FAILURE</code>
     * or <code>NOT_UNDERSTOOD</code> message is received from the DF (to
     * indicate some error condition) or if the supplied DF-Description is not
     * valid.
    ...
    /**
     * A default Agent Description is used which contains only the AID of this
     * agent.
     *
     * @exception FIPAException
     * @see #deregister(Agent a, AID dfName, DFAgentDescription dfd)
    ...
    /**
     * Deregisters a <code>DFAgentDescription</code> from the default DF. A
     * default DF-Description is used which contains only the AID of this agent.
     *
     * @exception FIPAException
     * @see #deregister(Agent a, AID dfName, DFAgentDescription dfd)
     *
     ...
     * <p>
     * It should be noted that, depending on the policy adopted by the DF, the
     * granted lease time for the modified registration can be shorter than the
     * requested one. This can be checked by looking at the DF-Description
     * actually registered by the DF that is made available as the return value
     * of this method. The <code>keepRegistered()</code> method can be used to
     * keep the registration valid until a given time.
     *
     * @param a is the Agent performing the request of modification
     * @param dfName The AID of the <b>DF</b> agent holding the data to be
     * changed.
     * @param dfd A <code>DFAgentDescription</code> object containing all new
     * data values;
     * @return the <code>DFAgentDescription</code> actually registered (after
     * the modification) by the DF
     * @exception FIPAException If a <code>REFUSE</code>, <code>FAILURE</code>
     * or <code>NOT_UNDERSTOOD</code> message is received from the DF (to
     * indicate some error condition) or if the supplied DF-Description is not
     * valid.
     ...
     * registered with a DF is kept registered until a given deadline. This
     * method is particularly useful when dealing with a DF agent that grants
     * limited lease time for agent registrations. The following piece of code
     ...
     * DF-Description must be kept valid. Use null to indicate an infinite time
     */
     ...
     * @param dfd A <code>DFAgentDescription</code> object containing data to
     * search for; this parameter is used as a template to match data against.
     * @param constraints of the search
     * @return An array of <code>DFAgentDescription</code> containing all found
     * items matching the given descriptor, subject to given search constraints
     * for search depth and result size.
     * @exception FIPAException If a <code>REFUSE</code>, <code>FAILURE</code>
     * or <code>NOT_UNDERSTOOD</code> message is received from the DF (to
     * indicate some error condition)
     */
     ...
     *
     * @exception FIPAException
     * @see #search(Agent a, AID dfName, DFAgentDescription dfd,
    ...
    /**
     * The default DF is used. The default SearchConstraints are used. According
     * to FIPA they are defaulted to null value for all slots.
     *
     * @exception FIPAException
     * @see #search(Agent a, AID dfName, DFAgentDescription dfd,
     * SearchConstraints constraints)
     ...
     * @param template A <code>DFAgentDescription</code> object that is used as
     * a template to identify the DF descriptions to search for.
     ...
     * @exception FIPAException If a <code>REFUSE</code>, <code>FAILURE</code>
     * or <code>NOT_UNDERSTOOD</code> message is received from the DF (to
     * indicate some error condition) or if the supplied DF-Description template
     * is not valid.
     */
    ...
    ///////////////////////////////////
    /**
     * Utility method that creates a suitable message to be used to REQUEST a DF
     * agent to perform a given action of the FIPA-Management-ontology.
     * <p>
     * This method can be fruitfully used in combination with the
     * <code>jade.proto.AchieveREInitiator</code> protocol and with the
     * <code>decodeDone()</code> and <code>decodeResult()</code> methods to
     * interact with a DF in a non-blocking way.
     *
    ...
    /**
     * Utility method that creates a suitable message to be used to SUBSCRIBE to
     * a DF agent in order to receive notifications when a new DF-Description
     * matching the indicated template is registererd with that DF.
     * <p>
     * This method can be fruitfully used in combination with the
     * <code>jade.proto.SubscriptionInitiator</code> protocol and with the
     * <code>createCancelMessage()</code> and <code>decodeNotification()</code>
     ...
     * @param template A <code>DFAgentDescription</code> object that is used as
     * a template to identify DF description that will be notified
     * @param constraints The constraints to limit the number of results to be
     * notified.
    ...
    /**
     * Utility method that creates a suitable message to be used to CANCEL a
     * subscription to a DF agent.
     *
     ...
     * @see jade.proto.SubscriptionInitiator
     * @see #createSubscriptionMessage(Agent, AID, DFAgentDescription,
     * SearchConstraints)
     * @see #decodeNotification(String)
     */
     ...
     * resulting from a <code>register</code> or <code>deregister</code> action
     * requested to a DF agent, extracting the <code>df-agent-description</code>
     * contained within.
     *
     * @return The <code>DFAgentDescription</code> object included in the "done"
     * expression used as the content of the INFORM message send back by the DF
     * in response to a REQUEST to perform a register, deregister or modify
     * action.
    ...
    /**
     * Process the content of the final <code>inform (result)</code> message
     * resulting from a <code>search</code> action requested to a DF agent,
     * extracting the array of <code>df-agent-description</code> contained
     * within.
     *
     * @return The <code>DFAgentDescription</code> objects (as an array)
     * included in the "result" expression used as the content of the INFORM
     * message send back by the DF in response to a REQUEST to perform a search
     * action.
     * @exception FIPAException If some error occurs while decoding
     */
    ...
    /**
     * Process the content of the <code>inform</code> message resulting from a
     * subscription with a DF agent, extracting the array of
     * <code>df-agent-description</code> objects contained within.
     *
     * @return The <code>DFAgentDescription</code> objects (as an array)
     * included in the "(= (iota...) ...)" expression used as the content of an
     * INFORM message sent back by the DF as a subscription notification.
     * @exception FIPAException If some error occurs while decoding
     */
    ...
    /**
     * The parser content has the form: df-agent-description ......)
    ...
    /**
     * The parser content has the form: service-description ......)
     * <possibly something else>
     */
    ...
    /**
     * The parser content has the form: property ......)
     * <possibly something else>
     */
    ...
    /**
     * The parser content has the form: multi-value-property ......)
     * <possibly something else>
     */
    ...
    /**
     * The parser content has the form: (sequence <val> <val> ......)
     * <possibly something else>
     * or (set <val> <val> ......) <possibly something else>
     */
    ...
    /**
     * S has the form: (sequence (DFD...) (DFD...)) <possibly something else>
     */
    ...
    //#MIDP_EXCLUDE_BEGIN 
    /**
     * In some cases it is more convenient to execute this tasks in a
     * non-blocking way. This method returns a non-blocking behaviour that can
     * be added to the queue of the agent behaviours, as usual, by using
     * <code>Agent.addBehaviour()</code>.
     * <p>
     * Several ways are available to get the result of this behaviour and the
     * programmer can select one according to his preferred programming style:
     * <ul>
     * <li>
     * call getLastMsg() and getSearchResults() where both throw a
     * NotYetReadyException if the task has not yet finished;
     * <li>create a SequentialBehaviour composed of two sub-behaviours: the
     * first subbehaviour is the returned RequestFIPAServiceBehaviour, while the
     * second one is application-dependent and is executed only when the first
     * is terminated;
     * <li>use directly the class RequestFIPAServiceBehaviour by extending it
     * and overriding all the handleXXX methods that handle the states of the
     * fipa-request interaction protocol.
     * </ul>
     *
     * @param a is the agent performing the task
     * @param dfName is the AID of the DF that should perform the requested
     * action
     ...
     * @param constraints are the search constraints (can be null if this is not
     * a search operation)
     * @return the behaviour to be added to the agent
     * @exception FIPAException A suitable exception can be thrown to indicate
     * some error condition locally discovered (e.g.the agentdescription is not
     * valid.)
     ...
     * The default DF is used.
     *
     * @see #getNonBlockingBehaviour(Agent a, AID dfName, String actionName,
     * DFAgentDescription dfd, SearchConstraints constraints)
     * @deprecated Use <code>AchieveREInitiator</code> instead
     *
     */
    ...
    /**
     * The default DF is used. the default SearchContraints are used. a default
     * AgentDescription is used, where only the agent AID is set.
     *
     * @see #getNonBlockingBehaviour(Agent a, AID dfName, String actionName,
     * DFAgentDescription dfd, SearchConstraints constraints)
     * @deprecated Use <code>AchieveREInitiator</code> instead
    ...
    /**
     * the default SearchContraints are used. a default AgentDescription is
     * used, where only the agent AID is set.
     *
     * @see #getNonBlockingBehaviour(Agent a, AID dfName, String actionName,
     * DFAgentDescription dfd, SearchConstraints constraints)
     * @deprecated Use <code>AchieveREInitiator</code> instead
