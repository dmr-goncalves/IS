...
/**
 * This class provides a set of static methods to communicate with a AMS Service
 * that complies with FIPA specifications. Notice that JADE calls automatically
 * the register and deregister methods with the default AMS respectively before
 * calling <code>Agent.setup()</code> method and just after
 * <code>Agent.takeDown()</code> method returns; so there is no need for a
 * normal programmer to call them. However, under certain circumstances, a
 * programmer might need to call its methods. To give some examples: when an
 * agent wishes to register with the AMS of a remote agent platform, or when an
 * agent wishes to modify its description by adding a private address to the set
 * of its addresses, ...
 * <p>
 ...
 * the default Search Constraints. Notice that all these methods blocks every
 * activity of the agent until the action (i.e.
 * register/deregister/modify/search) has been successfully executed or a
 * jade.domain.FIPAException exception has been thrown (e.g. because a FAILURE
 * message has been received by the AMS). In some cases, instead, it is more
 * convenient to execute this task in a non-blocking way. The method
 * getNonBlockingBehaviour() returns a non-blocking behaviour of type
 * RequestFIPAServiceBehaviour that can be added to the queue of the agent
 * behaviours, as usual, by using <code>Agent.addBehaviour()</code>.
 * <br>
 ...
 * @version $Date: 2012-09-10 10:53:21 +0200 (lun, 10 set 2012) $ $Revision:
 * 6588 $
 *
 */
public class AMSService extends FIPAService {
    ...
    /**
     * Register a AMSAgentDescription with a <b>AMS</b> agent. However, since
     * <b>AMS</b> registration and deregistration are automatic in JADE, this
     * method should not be used by application programmers to register with the
     * default AMS.
     *
     * @param a is the Agent performing the registration
     * @param AMSName The AID of the <b>AMS</b> agent to register with.
     * @param amsd A <code>AMSAgentDescriptor</code> object containing all data
     * necessary to the registration. If the Agent name is empty, than it is set
     * according to the <code>a</code> parameter. If the Agent state is empty,
     * than it is set to ACTIVE.
     * @exception FIPAException A suitable exception can be thrown when a
     * <code>refuse</code> or <code>failure</code> messages are received from
     * the AMS to indicate some error condition or when the method locally
     * discovers that the amsdescription is not valid.
     */
    ...
    /**
     * Deregister a AMSAgentDescription from a <b>AMS</b> agent. However, since
     * <b>AMS</b> registration and deregistration are automatic in JADE, this
     * method should not be used by application programmers to deregister with
     * the default AMS.
     *
     * @param AMSName The AID of the <b>AMS</b> agent to deregister from.
     * @param amsd A <code>AMSAgentDescription</code> object containing all data
    ...
    /**
     * A default AMSAgentDescription is used for this agent, where only AID and
     * state are set (state is set to ACTIVE).
     *
     * @see #deregister(Agent a, AID AMSName, AMSAgentDescription amsd)
     *
     */
    ...
    /**
     * A default AMSAgentDescription is used for this agent, where only AID and
     * state are set. The AID of the AMS is defaulted to the AMS of this
     * platform.
     *
     * @see #deregister(Agent a, AID AMSName, AMSAgentDescription amsd)
     *
     ...
     * agent.
     *
     * @param AMSName The GUID of the <b>AMS</b> agent holding the data to be
     * changed.
     * @param amsd The new <code>AMSAgentDescriptor</code> object that should
    ...
    /**
     * searches with the default AMS and the default SearchConstraints. The
     * default constraints specified by FIPA are max_results and max_depth both
     * unspecified and left to the choice of the responder AMS.
     *
     * @see #search(Agent,AID,AMSAgentDescription,SearchConstraints)
     *
     */
    ...
    /**
     * searches with the passed AMS by using the default SearchConstraints. The
     * default constraints specified by FIPA are max_results and max_depth both
     * unspecified and left to the choice of the responder AMS.
     *
     * @see #search(Agent,AID,AMSAgentDescription,SearchConstraints)
     ...
     * some error condition locally discovered (e.g.the amsdescription is not
     * valid.)
     ...
     *
     * @see #getNonBlockingBehaviour(Agent a, AID AMSName, String actionName,
     * AMSAgentDescription amsd, SearchConstraints constraints)
     *
     */
    ...
    /**
     * the default AMS is used. the default SearchContraints are used. a default
    ...
    /**
     * Extracts the receiver a message could not be delivered to from a FAILURE
     * message received by the AMS.
     *
