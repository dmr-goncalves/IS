...
/**
 * Standard <em>Agent Management System</em> agent. This class implements
 * <em><b>FIPA</b></em> <em>AMS</em> agent. <b>JADE</b>
 * applications cannot use this class directly, but interact with it through
 * <em>ACL</em> message passing.
 *
 * @author Giovanni Rimassa - Universita' di Parma
 * @author Giovanni Caire - TILAB
 * @version $Date: 2015-01-01 20:14:32 +0100 (gio, 01 gen 2015) $ $Revision:
 * 6734 $
 */
public class ams extends Agent /*implements AgentManager.Listener*/ {
     ...
     * reference to an Agent Platform implementation must be passed to it, this
     * constructor cannot be called from application code. Therefore, no other
     * <em>AMS</em> agent can be created beyond the default one.
     */
     ...
     * <li> The AMS internal max result limit if constraints.maxResults < 0 (the
     * FIPA specs requires it to be infinite, but for practical reason we prefer
     * to limit it) <li> constraints.maxResults otherwise
     * </ul>
     *
     */
    ...
    //////////////////////////////////////////////////////////////////
    /**
     * Inner calss RegisterToolBehaviour. This behaviour handles subscriptions
     * of tools i.e. agents to be notified about platform events.
     */
    ...
    /**
     * Inner calss DeregisterToolBehaviour. This behaviour handles tools
     * un-subscriptions.
     */
    ...
    /**
     * Inner interface Handler. Perform additional operations related to a given
     * platform event
     */
    ...
    /**
     * Inner class EventManager. This behaviour notifies - all registered tools
     * about all platform events - the agent that had requested the AMS to
     * perform an action about the platform event forced by that action. Note
     * that this is done only for actions that produce an "asynchronous event".
     */
     ...
     *
     * public void bornAgent(PlatformEvent ev) { if
     * (logger.isLoggable(Logger.CONFIG)) logger.log(Logger.CONFIG,
     * ev.toString()); ContainerID cid = ev.getContainer(); AID agentID =
     * ev.getAgent(); String ownership = ev.getNewOwnership();
     *
     * BornAgent ba = new BornAgent(); ba.setAgent(agentID); ba.setWhere(cid);
     * ba.setState(AMSAgentDescription.ACTIVE); ba.setOwnership(ownership);
     * ba.setClassName((String)
     * agentID.getAllUserDefinedSlot().get(AID.AGENT_CLASSNAME));
     *
     * EventRecord er = new EventRecord(ba, here()); er.setWhen(ev.getTime());
     * eventQueue.put(er); }
     */
    /**
     * Put a DeadAgent event in the AMS event queue
     *
     * public void deadAgent(PlatformEvent ev) { if
     * (logger.isLoggable(Logger.CONFIG)) logger.log(Logger.CONFIG,
     * ev.toString()); ContainerID cid = ev.getContainer(); AID agentID =
     * ev.getAgent();
     *
     * DeadAgent da = new DeadAgent(); da.setAgent(agentID); da.setWhere(cid);
     * if (ev.getContainerRemoved()) { da.setContainerRemoved(new
     * Boolean(true)); }
     *
     * EventRecord er = new EventRecord(da, here()); er.setWhen(ev.getTime());
     * eventQueue.put(er); }
     */
    /**
     * Put a SuspendedAgent event in the AMS event queue
     *
     * public void suspendedAgent(PlatformEvent ev) { if
     * (logger.isLoggable(Logger.CONFIG)) logger.log(Logger.CONFIG,
     * ev.toString()); ContainerID cid = ev.getContainer(); AID name =
     * ev.getAgent();
     *
     * SuspendedAgent sa = new SuspendedAgent(); sa.setAgent(name);
     * sa.setWhere(cid);
     *
     * EventRecord er = new EventRecord(sa, here()); er.setWhen(ev.getTime());
     * eventQueue.put(er); }
     */
    /**
     * Put a ResumedAgent event in the AMS event queue
     *
     * public void resumedAgent(PlatformEvent ev) { if
     * (logger.isLoggable(Logger.CONFIG)) logger.log(Logger.CONFIG,
     * ev.toString()); ContainerID cid = ev.getContainer(); AID name =
     * ev.getAgent();
     *
     * ResumedAgent ra = new ResumedAgent(); ra.setAgent(name);
     * ra.setWhere(cid);
     *
     * EventRecord er = new EventRecord(ra, here()); er.setWhen(ev.getTime());
     * eventQueue.put(er); }
     */
    /**
     * Put a FrozenAgent event in the AMS event queue
     *
     * public void frozenAgent(PlatformEvent ev) { if
     * (logger.isLoggable(Logger.CONFIG)) logger.log(Logger.CONFIG,
     * ev.toString()); ContainerID cid = ev.getContainer(); AID name =
     * ev.getAgent(); ContainerID bufferContainer = ev.getNewContainer();
     *
     * FrozenAgent fa = new FrozenAgent(); fa.setAgent(name); fa.setWhere(cid);
     * fa.setBufferContainer(bufferContainer);
     *
     * EventRecord er = new EventRecord(fa, here()); er.setWhen(ev.getTime());
     * eventQueue.put(er); }
     */
    /**
     * Put a ThawedAgent event in the AMS event queue
     *
     * public void thawedAgent(PlatformEvent ev) { if
     * (logger.isLoggable(Logger.CONFIG)) logger.log(Logger.CONFIG,
     * ev.toString()); ContainerID cid = ev.getContainer(); AID name =
     * ev.getAgent(); ContainerID bufferContainer = ev.getNewContainer();
     *
     * ThawedAgent ta = new ThawedAgent(); ta.setAgent(name); ta.setWhere(cid);
     * ta.setBufferContainer(bufferContainer);
     *
     * EventRecord er = new EventRecord(ta, here()); er.setWhen(ev.getTime());
     * eventQueue.put(er); }
     */
    /**
     * Put a MovedAgent event in the AMS event queue
     *
     * public void movedAgent(PlatformEvent ev) { if
     * (logger.isLoggable(Logger.CONFIG)) logger.log(Logger.CONFIG,
     * ev.toString()); ContainerID from = ev.getContainer(); ContainerID to =
     * ev.getNewContainer(); AID agentID = ev.getAgent();
     *
     * MovedAgent ma = new MovedAgent(); ma.setAgent(agentID); ma.setFrom(from);
     * ma.setTo(to);
     *
     * EventRecord er = new EventRecord(ma, here()); er.setWhen(ev.getTime());
     * eventQueue.put(er); }
     */
    /**
     * Put a ChangedAgentOwnership event in the AMS event queue
     *
     * public void changedAgentPrincipal(PlatformEvent ev) { if
     * (logger.isLoggable(Logger.CONFIG)) logger.log(Logger.CONFIG,
     * ev.toString()); ContainerID cid = ev.getContainer(); AID name =
     * ev.getAgent();
     *
     * ChangedAgentOwnership cao = new ChangedAgentOwnership();
     * cao.setAgent(name); cao.setWhere(cid); cao.setFrom(ev.getOldOwnership());
     * cao.setTo(ev.getNewOwnership());
     *
     * EventRecord er = new EventRecord(cao, here()); er.setWhen(ev.getTime());
     * eventQueue.put(er); }
     */
    /**
     * Put an AddedContainer event in the AMS event queue
     *
     * public void addedContainer(PlatformEvent ev) { if
     * (logger.isLoggable(Logger.CONFIG)) logger.log(Logger.CONFIG,
     * ev.toString()); ContainerID cid = ev.getContainer(); String name =
     * cid.getName();
     *
     * AddedContainer ac = new AddedContainer(); ac.setContainer(cid);
     *
     * EventRecord er = new EventRecord(ac, here()); er.setWhen(ev.getTime());
     * eventQueue.put(er); }
     */
    /**
     * Put a RemovedContainer event in the AMS event queue
     *
     * public void removedContainer(PlatformEvent ev) { if
     * (logger.isLoggable(Logger.CONFIG)) logger.log(Logger.CONFIG,
     * ev.toString()); ContainerID cid = ev.getContainer(); String name =
     * cid.getName();
     *
     * RemovedContainer rc = new RemovedContainer(); rc.setContainer(cid);
     *
     * EventRecord er = new EventRecord(rc, here()); er.setWhen(ev.getTime());
     * eventQueue.put(er); }
     */
    /**
     * Put a XXX event in the AMS event queue
     *
     * public synchronized void changedContainerPrincipal(PlatformEvent ev) { if
     * (logger.isLoggable(Logger.CONFIG)) logger.log(Logger.CONFIG,
     * ev.toString()); // FIXME: There is no element in the
     * IntrospectionOntology // corresponding to this event }
     */
    /**
     * Put a AddedMTP event in the AMS event queue
     *
     * public synchronized void addedMTP(MTPEvent ev) { if
     * (logger.isLoggable(Logger.CONFIG)) logger.log(Logger.CONFIG, "MTPEvent
     * [added MTP]"); Channel ch = ev.getChannel(); ContainerID cid =
     * ev.getPlace(); String proto = ch.getProtocol(); String address =
     * ch.getAddress();
     *
     * // Generate a suitable AMS event AddedMTP amtp = new AddedMTP();
     * amtp.setAddress(address); amtp.setProto(proto); amtp.setWhere(cid);
     *
     * EventRecord er = new EventRecord(amtp, here()); er.setWhen(ev.getTime());
     * eventQueue.put(er);
     *
     * // The PlatformDescription has changed --> Generate a suitable event
     * PlatformDescription ap = new PlatformDescription();
     * ap.setPlatform(getDescriptionAction(null)); er = new EventRecord(ap,
     * here()); er.setWhen(ev.getTime()); eventQueue.put(er); }
     */
    /**
     * Put a RemovedMTP event in the AMS event queue
     *
     * public synchronized void removedMTP(MTPEvent ev) { if
     * (logger.isLoggable(Logger.CONFIG)) logger.log(Logger.CONFIG, "MTPEvent
     * [removed MTP]"); Channel ch = ev.getChannel(); ContainerID cid =
     * ev.getPlace(); String proto = ch.getProtocol(); String address =
     * ch.getAddress();
     *
     * RemovedMTP rmtp = new RemovedMTP(); rmtp.setAddress(address);
     * rmtp.setProto(proto); rmtp.setWhere(cid);
     *
     * EventRecord er = new EventRecord(rmtp, here()); er.setWhen(ev.getTime());
     * eventQueue.put(er);
     *
     * // The PlatformDescription has changed --> Generate a suitable event
     * PlatformDescription ap = new PlatformDescription();
    ...
    /**
     * Store a notification message to be sent at a later time. Package-scoped
     * as it is called by the AMSJadeAgentManagementBehaviour
     */
    ...
    /**
     * This method is executed by an auxiliary thread started by the AMS to
     * perform certain actions (createAgent, killContainer) asynchronously
     */
